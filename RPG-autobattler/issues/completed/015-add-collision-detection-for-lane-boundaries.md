# Issue #015: Add Collision Detection for Lane Boundaries

## Current Behavior
No collision detection system exists to keep units within their designated lanes and sub-paths.

## Intended Behavior
A robust collision detection system should ensure units stay within lane boundaries and handle interactions with lane edges appropriately.
...
### Collision Features
1. **Boundary Generation**: Automatic creation of collision boundaries for sub-paths
2. **Position Validation**: Check if units are within valid areas
3. **Position Correction**: Move units back into bounds when needed
4. **Available Space Detection**: Find valid positions for unit placement
5. **Debug Visualization**: Optional boundary rendering for debugging

### Considerations
- Optimize collision checks for performance with many units
- Handle edge cases like curved paths and intersections
- Consider soft vs hard boundaries (guidance vs walls)
- Plan for dynamic boundary modifications
- Add tolerance for slight boundary violations

### Tool Suggestions
- Use Write tool to create collision system
- Test boundary detection with various path shapes
- Verify position correction works smoothly
- Check performance with many simultaneous collision checks

### Acceptance Criteria
- [ ] Units stay within their assigned sub-path boundaries
- [ ] Collision detection works for curved and complex paths
- [ ] Position correction maintains smooth movement
- [ ] System performs well with many units
