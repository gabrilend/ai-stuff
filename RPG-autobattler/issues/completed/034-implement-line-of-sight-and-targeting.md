# Issue #034: Implement Line-of-Sight and Targeting

## Current Behavior
Ranged units can attack any enemy within range regardless of obstacles or line-of-sight blocking, making combat unrealistic.

## Intended Behavior
Ranged units should only be able to target enemies they can actually see, with proper line-of-sight calculations that account for obstacles, terrain, and other units.

## Implementation Details

### Line-of-Sight System (src/systems/line_of_sight_system.lua)
```lua
    ...
    if #visible_targets > 0 then
        ...
        if best_target then
            ...
            if unit_data.state ~= "combat" then
                initiate_combat_engagement(unit_id, best_target)
            end
        end
    else
...
local function create_los_debug_visualization(from_unit_id, to_unit_id, los_result)
    ...
    DebugSystem:add_debug_effect(debug_effect)
end
-- }}}
...
1. **Raycast System**: Accurate line-of-sight calculations between units
2. **Obstacle Detection**: Recognition of units, terrain, and boundaries blocking sight
3. **Partial Cover**: Support for targets that are partially obscured
4. **Target Selection**: Automatic selection of visible targets
5. **Debug Visualization**: Visual debugging for line-of-sight rays

### LOS Mechanics
- **Clear Sight**: Direct line with no obstructions
- **Partial Cover**: Target partially blocked with accuracy penalty
- **Full Cover**: Target completely blocked, cannot target
- **Dynamic Updates**: Real-time visibility checking as units move

### Blocking Elements
- **Other Units**: Living units block line-of-sight
- **Terrain**: Environmental obstacles and barriers
- **Lane Boundaries**: Walls and structural elements
- **Size-Based**: Larger units provide more cover

### Tool Suggestions
- Use Write tool to create line-of-sight system
- Test with units positioned behind obstacles
- Verify partial cover calculations and penalties
- Check target selection with multiple visible enemies

### Acceptance Criteria
- [ ] Ranged units only target enemies they can see
- [ ] Obstacles properly block line-of-sight
- [ ] Partial cover reduces accuracy appropriately
- [ ] Target selection prioritizes visible enemies
- [ ] Debug visualization aids in system development