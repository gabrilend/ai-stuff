...
## Current Behavior
Ranged units use basic combat positioning but lack specialized behaviors for maintaining optimal firing distance and avoiding close combat.

## Intended Behavior
Ranged units should actively maintain optimal firing distance, retreat when threatened by melee units, and position themselves for maximum effectiveness while avoiding close combat engagement.

## Implementation Details

### Ranged Behavior System (src/systems/ranged_behavior_system.lua)
```lua
...
local function schedule_return_to_stationary(unit_id, delay)
    ...
    BehaviorScheduler:schedule_action(unit_id, "return_to_stationary", schedule_time, function()
        local moveable = EntityManager:get_component(unit_id, "moveable")
        local unit_data = EntityManager:get_component(unit_id, "unit")
...
### Ranged Behavior Features
1. **Distance Management**: Maintain optimal firing range automatically
2. **Threat Assessment**: Evaluate immediate dangers and positioning
3. **Emergency Retreat**: Fast escape from melee threats
4. **Tactical Repositioning**: Move to better firing positions
5. **Micro-Adjustments**: Small movements to avoid predictability

### Behavioral States
- **Firing**: Stationary for accurate shooting
- **Emergency Retreat**: Fast escape from immediate danger
- **Repositioning**: Moving to optimal position
- **Micro-Adjustment**: Small positional tweaks

### Decision Making
- Prioritize survival over damage output
- Maintain minimum safe distance from melee units
- Position for maximum effectiveness against targets
- Adapt behavior based on threat assessment

### Tool Suggestions
- Use Write tool to create ranged behavior system
- Test with various melee vs ranged scenarios
- Verify retreat and repositioning behaviors
- Check optimal positioning calculations

### Acceptance Criteria
- [ ] Ranged units maintain proper distance from enemies
- [ ] Emergency retreat activates when threatened by melee
- [ ] Tactical repositioning improves firing effectiveness
- [ ] Micro-adjustments prevent static positioning
- [ ] Visual feedback clearly shows ranged unit states