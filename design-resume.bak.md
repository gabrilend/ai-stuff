...
Creates **interface summary files** that dramatically reduce token load:
...
> "prefer error messages and breaking functionality over fallbacks. Be sure to notify the user every time a fallback is used, and create a new issue file to resolve any fallbacks if they are triggered when testing..."
...
> "if the user asks design questions, ask them questions back. try to get them to think about solving problems... 'why is this behavior still occurring?' 'here are two equivalent facts. how could it be so?'"

**Category:** Agent Behavior
**Multi-Agent Relevance:** HIGH

Agents **teach through questioning**:
...
[FIXME]: make this more about design. be sure to highlight the difference between informational questions ("why does this library do things this way") and between design questions ("how can we structure this part of the program to do this thing this way")
Do:
```
User: Why is X happening?
Agent: What have you ruled out so far?
       What would you expect to see if it were Y?
       Have you considered whether Z could cause this?
```

**Integration Proposal:** "Socratic Mode" for debugging:
1. Ask clarifying questions before answering
2. Guide user to discover answers
3. Direct answers only when stuck
4. Track approaches user has tried
...
[FIXME]: can you write some basic lua code here instead of the pseudo-code? try and include something that has the same words repeated, so we can align them. Notice how user_input and validated_input are aligned such that input is lined up vertically - try and make opportunities for that. Also, have some arguments, and then do something else to those arguments on the next line so we can line them up and show the flow of data through the program.
...
The diagonal flow with preserved terms shows transformation chain.
...
[FIXME]: let's do progress files, since the architecture is already in place for them.
...
[FIXME]: can you add something here and elsewhere related to the info.md files that describes them like a header file? because that's essentially what they are, except for the human/agent instead of the compiler.
Layer 2: Info.md Summaries
├── One per source file
...
- [ ] **OB tracking and insight propagation**
  - Link issues to root causes
  - Propagate insights to related issues
  - Maintain causal graph
  - Directive: D-020
...
| D-010 | Issue Cross-Referencing | Knowledge Propagation | Critical |
...
| D-020 | OB Tracking | Bug Tracking | High |
...
| D-058 | Issue Deduplication | Issue Management | Medium |
