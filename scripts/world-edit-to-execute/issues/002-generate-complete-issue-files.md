...
Now use the Write tool to create the actual issue files.

For each sub-issue, create a complete issue file with:
...
Use the Write tool directly to create each file. The tool is available to you.
...
Claude uses the Write tool directly to create each file with complete content.
Each file is implementation-ready when created.
