...
# Each response is kept beside the issue, in issues/analysis/<issue>-split-analysis.md,
# never inside the issue file itself.
#
# Why the analysis lives beside the issue and not in it: an issue file is a
# blueprint for building the software, read by the next person to build it. A
# model's "should this be split?" deliberation, dated and appended, is a work
# record, and it made every analysed issue longer and harder to read. The
# splitting advice that matters survives as the sub-issue files it produced;
# the parent issue gains only the "Generated Sub-Issues" list naming them.
# Issues analysed by earlier versions still carry their analysis inside the
# file; those are still read (with a warning saying so), and -C strips them out.
#
# The project is named explicitly (first argument or -d). An earlier version
# defaulted to world-edit-to-execute, so running it bare from any other project
# sent that project's issues to Claude; a shared tool has no right default.
#
# Auto-implement (-A) runs Claude Code with its normal permission checks. An
# earlier version passed the flag that switches every check off, which let an
# unattended run (-X) write and execute anything with nobody asked.
#
# Behavior:
...
#
# Usage:
#   ./issue-splitter.sh <project-dir> [options]
#   ./issue-splitter.sh <project-dir> -I   (interactive mode)
#   ./issue-splitter.sh --dir /path        (same as naming it first)
#
# Options:
#   -d, --dir <path>      Project directory (required; or give it as the first argument)
...
# {{{ Configuration
# The project to work on. Deliberately empty: it must be named on the command
# line (first argument, or -d), because every run sends that project's issues
# to Claude and there is no project this shared script should assume.
DIR=""
...
process_issue_parallel() {
    ...
    if timeout 300 claude -p "$prompt" > "$output_file" 2>&1; then
        ...
        # Append analysis to the issue's analysis file (beside it, not in it)
        mkdir -p "${ISSUES_DIR}/analysis"
        {
            echo ""
            echo "---"
            echo ""
...
# {{{ print_help
print_help() {
    # Print the header from "Behavior:" through the last option line, so the
    # help stays whatever the header says, however long its preamble grows.
    sed -n '/^# Behavior:/,/^#   -h, --help/p' "$0" | sed 's/^# \{0,1\}//'
}
# }}}
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -*)
                ...
                ;;
            *)
                # A bare word is the project folder, the house convention for
                # overriding a script's DIR.
                DIR="$1"
                ISSUES_DIR="${DIR}/issues"
                ARCHIVE_DIR="${DIR}/issues/analysis"
                shift
    ...
    if [[ -z "$DIR" ]]; then
        error "Name the project: issue-splitter.sh <project-dir> [options]  (or -d <project-dir>)"
    fi
}
# }}}
...
# {{{ analysis_file_of
# Where the splitter keeps its analysis of one issue: beside the issue, in
# issues/analysis/, never inside it (see the header for why). Every new
# analysis, feedback analysis and structure review is appended here.
analysis_file_of() {
    local issue_path="$1"
    local name
    name=$(basename "$issue_path" .md)
    echo "${ISSUES_DIR}/analysis/${name}-split-analysis.md"
}
# }}}

# {{{ analysis_source_of
# The file to READ an issue's analysis from. Normally its analysis file. An
# issue analysed by an earlier version of this script carries the analysis
# inside the issue file instead; that is still read, so old work is not
# redone, but a warning names the file each time -- run with -C to move those
# sections out of the issue. Prints the path; nothing if there is no analysis.
analysis_source_of() {
    local issue_path="$1"
    local analysis_file
    analysis_file=$(analysis_file_of "$issue_path")
    if [[ -f "$analysis_file" ]]; then
        echo "$analysis_file"
    elif grep -qE "^## (Sub-Issue Analysis|Initial Analysis|Structure Review)( \(Feedback Mode\))?$" "$issue_path"; then
        echo "[WARNING] reading analysis from inside $(basename "$issue_path") (older layout; -C moves it out)" >&2
        echo "$issue_path"
    fi
}
# }}}
...
has_subissue_analysis() {
    local file
    file=$(analysis_source_of "$1" 2>/dev/null)
    [[ -n "$file" ]] || return 1
    ...
    grep -qE "^## Sub-Issue Analysis( \(Feedback Mode\))?$" "$file"
}
# }}}
...
get_review_content() {
    ...
    # If no analysis found, show the issue itself
    if [[ -z "$content" ]]; then
        content=$(cat "$file")
    fi
...
rename_analysis_to_initial() {
    local issue_path
    issue_path=$(analysis_source_of "$1" 2>/dev/null)
    [[ -n "$issue_path" ]] || return 0
...
$response" "$user_response")
    ...
    # Append the final analysis to the issue's analysis file (beside it, not in it)
    mkdir -p "${ISSUES_DIR}/analysis"
    {
        echo ""
        echo "---"
    ...
    log "  Analysis written to issues/analysis/"
...
# Move analysis sections written by older versions of this script out of an
# issue file and into the issue's analysis file (issues/analysis/), leaving
# the issue as the blueprint it should be.
# Sections moved:
#   - ## Sub-Issue Analysis (and its "(Feedback Mode)" form)
#   - ## Initial Analysis
#   - ## Structure Review
# Kept in the issue: ## Generated Sub-Issues -- the list of sub-issue files
# is part of the blueprint (it names the pieces the issue was split into).
#
# Where a moved section ends: at the next heading that belongs to the issue
# itself (Implementation Notes, Notes, Related Documents, Acceptance
# Criteria, Generated Sub-Issues), at another analysis heading, or at the end
# of the file. It does NOT end at any "## " line, because the model's answers
# contain their own "## Recommendation"-style headings, and stopping there
# left the tail of every analysis behind in the issue.
clear_analysis_from_issue() {
    local issue_path="$1"
    local basename
    basename=$(basename "$issue_path")

    log "Moving analysis out of: $basename"
    ...
    local analysis_start='^## (Sub-Issue Analysis( \(Feedback Mode\))?|Initial Analysis|Structure Review)$'
    local issue_heading='^## (Implementation Notes|Notes|Related Documents|Acceptance Criteria|Generated Sub-Issues)'

    # First, extract the analysis sections
    local analysis_content
    analysis_content=$(awk -v start="$analysis_start" -v stop="$issue_heading" '
    $0 ~ start { skip = 1; print; next }
    $0 ~ stop  { skip = 0 }
    skip == 1  { print }
    ' "$issue_path")
    ...
    if [[ "$DRY_RUN" == true ]]; then
        log "  [DRY RUN] Would move analysis sections to issues/analysis/"
        return 0
    fi
    ...
    {
        echo ""
        echo "<!-- moved out of ${basename} on $(date '+%Y-%m-%d %H:%M') -->"
        echo ""
        echo "$analysis_content"
    } >> "$analysis_file"
    log "  Moved to: issues/analysis/$(basename "$analysis_file")"

    # Write the issue without those sections
    local temp_file
    temp_file=$(mktemp)
    awk -v start="$analysis_start" -v stop="$issue_heading" '
    $0 ~ start { skip = 1; next }
    $0 ~ stop  { skip = 0 }
    skip == 0  { print }
    ' "$issue_path" > "$temp_file"

    # The old layout put a "---" rule before each appended section; drop the
    # rules and blank lines left dangling at the end of the file.
    local trimmed_file
    trimmed_file=$(mktemp)
    tac "$temp_file" | awk 'trimming && /^(\s*|---)$/ { next } { trimming = 0; print }' trimming=1 | tac > "$trimmed_file"
    rm -f "$temp_file"

    mv "$trimmed_file" "$issue_path"
    log "  Issue now holds only its own sections"
    return 0
}
# }}}
...
review_root_issue() {
    ...
    local response
    if response=$(call_claude "$prompt" 300); then
        # Append review to the root issue's analysis file (beside it, not in it)
        mkdir -p "${ISSUES_DIR}/analysis"
        {
            echo ""
            echo "---"
            ...
            echo ""
            echo "$response"
        } >> "$(analysis_file_of "$root_path")"

        log "  Review written to issues/analysis/"
...
# {{{ parse_analysis
parse_analysis() {
    # Read from the analysis file (or, for an issue analysed by an older
    # version, from inside the issue -- see analysis_source_of).
    local issue_path
    issue_path=$(analysis_source_of "$1")
    [[ -n "$issue_path" ]] || return 0
...
auto_implement_issue() {
    ...
    if [[ "$EXECUTE_ALL" != true ]]; then
        echo ""
        echo "About to open a Claude Code session to implement: $basename"
        echo "It asks before acting, under your usual permission settings."
        echo ""
    ...
    # Open an interactive session with the implementation prompt as its first
    # message. Interactive rather than piped: a piped prompt runs Claude with
    # no one to answer its permission questions, which is why the earlier
    # version switched every check off. Here the person at the terminal
    # answers them, and the permission mode in their settings applies.
    log "  Opening Claude Code..."
    local exit_code=0
