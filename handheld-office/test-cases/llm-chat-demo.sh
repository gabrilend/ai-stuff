...
# Step 1: Start daemon
log "📡 Starting daemon..."
env RUST_LOG=info ./target/release/daemon > "$LOG_DIR/daemon-llm.log" 2>&1 &
DAEMON_PID=$!
sleep 2

# Step 2: Start LLM service 
log "🤖 Starting LLM service (with fallback AI)..."
env RUST_LOG=info ./target/release/desktop-llm > "$LOG_DIR/llm-service.log" 2>&1 &
LLM_PID=$!
sleep 2
...
./target/release/handheld < "$LOG_DIR/llm_commands.txt" > "$LOG_DIR/llm-client.log" 2>&1
...
log ""
log "📁 Detailed logs in: test-cases/logs/"