...
- **127.0.0.1 binding** for laptop daemon (localhost only)
- **WiFi Direct P2P** is the ONLY allowed method for Anbernic device communication
- **Air-gapped architecture** - Anbernic devices have no internet access
...
## MMO Engine - Peer-to-Peer Swarm Networking (`src/games/src/mmo_engine.rs`)
...
#### WiFi Direct P2P for Player Discovery

Unlike traditional MMO servers that maintain centralized player databases, the Anbernic MMO system uses **WiFi Direct P2P mesh networking** where each handheld device discovers peers through air-gapped device-to-device communication.

```rust
// Network discovery pseudocode (inferred from architecture)
impl NetworkManager {
    async fn discover_peers(&self) -> Vec<PeerInfo> {
        // 1. WiFi Direct device discovery (air-gapped)
        // 2. Establish encrypted P2P connections via relationship pairing
        // 3. Exchange world state chunks through mesh network
        // 4. Maintain peer list through P2P gossip (no internet required)
    }
...
The system supports **mesh networking** where handheld devices can relay messages through each other in the P2P network without requiring internet connectivity.
...
pub struct DaemonConfig {
    ...
    pub bind_address: String,         // Default: "127.0.0.1" (laptop daemon localhost only)
...
### 1. Enhanced WiFi Direct P2P

Improved air-gapped peer-to-peer connections between handheld devices:

```rust
// Enhanced WiFi Direct implementation
impl WiFiDirectNetwork {
    async fn establish_secure_p2p_connection(&mut self, peer_id: DeviceId) -> Result<P2PConnection, NetworkError> {
        // WiFi Direct device discovery
        // Relationship-based encryption setup
        // Direct encrypted data channels (no internet required)
    }
}
```
