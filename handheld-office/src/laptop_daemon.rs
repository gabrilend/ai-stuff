...
use crate::ai_image_service::AIImageService;
use crate::desktop_llm::DesktopLlmService;
...
impl LaptopDaemon {
    ...
    pub fn new(config: LaptopDaemonConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if config.llm_enabled {
            ...
            let desktop_llm = Arc::new(DesktopLlmService::new());
            bytecode_executor.set_llm_provider(desktop_llm);
        }
        if config.image_generation_enabled {
            info!("Image generation services enabled via bytecode interface (internet proxy mode)");
            let ai_image_service = Arc::new(AIImageService::new(config.models_path.clone(), config.output_path.clone())?);
            bytecode_executor.set_image_provider(ai_image_service);
        }
