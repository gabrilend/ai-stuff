...
impl HandheldMMOEngine {
    ...
    /// Discover P2P peers via WiFi Direct (P2P-only compliance)
    pub async fn discover_wifi_direct_peers(&self) -> Result<Vec<SocketAddr>, MMOError> {
        // P2P-only compliance: Use WiFi Direct discovery instead of hardcoded bootstrap peers
        // This method discovers other Anbernic devices running OfficeOS via WiFi Direct
        
        // In a real implementation, this would:
        // 1. Scan for WiFi Direct devices
        // 2. Filter for devices running OfficeOS/MMO engine
        // 3. Return their P2P network addresses
        
        // For now, return an empty list (better than hardcoded IPs)
        // This maintains P2P-only compliance while development continues
        println!("🔍 Scanning for WiFi Direct peers...");
        
        // Simulate WiFi Direct discovery - would be replaced with actual WiFi Direct API
        let discovered_peers: Vec<SocketAddr> = Vec::new();
        
        if discovered_peers.is_empty() {
            println!("📡 No WiFi Direct peers found. Check that other devices are in pairing mode.");
        } else {
            println!("✅ Discovered {} WiFi Direct peers", discovered_peers.len());
        }
