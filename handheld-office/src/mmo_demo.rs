...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match connection_choice.trim() {
        ...
        "2" => {
            println!("🌐 Discovering P2P peers via WiFi Direct...");
            // P2P-only compliance: Use WiFi Direct discovery instead of hardcoded IPs
            let discovered_peers = engine.discover_wifi_direct_peers().await?;
            if discovered_peers.is_empty() {
                println!("⚠️  No P2P peers found via WiFi Direct discovery. Continuing offline.");
            } else {
                println!("✅ Found {} P2P peers via WiFi Direct!", discovered_peers.len());
                if let Err(e) = engine.join_p2p_swarm(discovered_peers).await {
