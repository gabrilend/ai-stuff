...
env RUST_LOG=debug ./files/target/release/daemon > "$LOG_DIR/stress-daemon.log" 2>&1 &
DAEMON_PID=$!
sleep 2
...
for i in $(seq 1 $NUM_CLIENTS); do
    ...
    env RUST_LOG=info DEVICE_ID="anbernic_$i" ./files/target/release/handheld \
...
log ""
log "📁 Detailed analysis in: examples/test-cases/logs/stress-*"
