...
source "examples/test-cases/test_common.sh"

LOG_DIR="examples/test-cases/logs"
mkdir -p "$LOG_DIR"

# Validate environment and check for required binaries
if ! validate_test_environment; then
    exit 1
fi

if ! check_binaries_exist "daemon" "desktop-llm" "handheld"; then
...
env RUST_LOG=info ./files/target/release/daemon > "$LOG_DIR/daemon.log" 2>&1 &
DAEMON_PID=$!
sleep 2
...
env RUST_LOG=info ./files/target/release/desktop-llm > "$LOG_DIR/llm.log" 2>&1 &
LLM_PID=$!
sleep 1
...
env RUST_LOG=info ./files/target/release/handheld < "$LOG_DIR/alice_input.txt" > "$LOG_DIR/alice.log" 2>&1 &
CLIENT1_PID=$!
sleep 2
...
env RUST_LOG=info ./files/target/release/handheld < "$LOG_DIR/bob_input.txt" > "$LOG_DIR/bob.log" 2>&1 &
CLIENT2_PID=$!
sleep 3
...
log ""
log "📁 Full logs saved in: examples/test-cases/logs/"
