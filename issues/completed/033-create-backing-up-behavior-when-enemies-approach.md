# Issue #033: Create Backing-Up Behavior When Enemies Approach

## Current Behavior
Ranged units have basic retreat behavior but lack sophisticated backing-up mechanics that maintain firing capability while retreating.

## Intended Behavior
Ranged units should intelligently back away from approaching enemies while continuing to fire, using kiting tactics to maintain optimal engagement distance.

## Implementation Details

### Backing-Up Behavior System (src/systems/backing_up_behavior_system.lua)
```lua
...
local function execute_backing_fire(unit_id, target_id)
    ...
    local projectile_id = create_projectile(unit_id, target_id, "basic_arrow")
    ...
    if unit_data.combat_target then
        ...
        if target_still_valid then
            ...
            if #enemy_units > 0 then
                ...
                if new_target then
                    unit_data.combat_target = new_target.unit_id
                    stop_backing_up_behavior(unit_id)
                end
            end
        end
...
### Backing-Up Features
1. **Threat Analysis**: Detect and evaluate approaching enemies
2. **Dynamic Retreat**: Intelligent backing away while maintaining combat
3. **Fire While Moving**: Continue attacking with reduced accuracy
4. **Path Finding**: Find clear retreat routes around obstacles
5. **Re-engagement**: Return to optimal positioning when safe

### Tactical Behaviors
- **Kiting**: Maintain distance while continuing to deal damage
- **Emergency Retreat**: Fast escape from immediate danger
- **Lateral Movement**: Side-stepping when direct retreat is blocked
- **Controlled Re-engagement**: Return to fight when threats diminish

### Combat Effectiveness
- Reduced accuracy while moving maintains balance
- Continued damage output during retreat
- Smart positioning to avoid being cornered
- Automatic threat re-evaluation

### Tool Suggestions
- Use Write tool to create backing-up behavior system
- Test with aggressive melee units pursuing ranged units
- Verify firing accuracy penalties and movement coordination
- Check retreat path finding and obstacle avoidance

### Acceptance Criteria
- [ ] Ranged units back away from approaching melee threats
- [ ] Units continue firing while retreating with accuracy penalty
- [ ] Retreat paths avoid obstacles and lane boundaries
- [ ] Re-engagement occurs when threats diminish
- [ ] Kiting behavior maintains effective combat distance