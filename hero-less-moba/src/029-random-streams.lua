...
function M.make_set(seed)
  return {
    ...
    -- comes from a stream made per wave out of the match seed and the wave's own
    -- number -- see the formations module. Everything in this set is shared by
    -- everything that reads it, which is right for a draw nobody else can influence
    -- and wrong for one whose answer would otherwise depend on how many other waves
