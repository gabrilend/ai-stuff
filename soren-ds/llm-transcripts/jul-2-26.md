...
**What "chips" are.** Probes are *fire-and-log*: a tiny declarative DSL
(`W`/`R`/`DELAY`/`EXPECT`…) baked into the image, swept at boot, verdict
machine-readable. Chips are the opposite shape — *interactive C*: they drive a
device, then ask a human "did you see/hear/feel it?" You can't express a menu or
a read-a-keypress in the probe DSL, so chips are hand-written C in a new file,
sharing only the underlying drivers (exactly what issue 115 predicted).


**The missing primitive.** The CDC-ACM console is **output-only** today —
`debug_write` pushes bytes to the host, but the `EP_BULK_OUT` (host→device)
endpoint is configured yet *never read*. A menu needs to read the developer's
selection. So "get I/O working" = **build the console read path**. That's the
foundation both chips (I/O-validation, and the later probe-selector) stand on.


**Scope of this first pass**, kept dormant (not wired into boot, `#ifdef
SOREN_DEBUG` so production images carry none of it):

1. Console **input** in `011-cdc-acm.c` — `console_getchar()` /
   `console_read()`, mirroring `debug_write`'s TRB machinery on the OUT
   endpoint.

2. New `020-chips.c` — the chips category: a reusable **menu** primitive
   (print options, read a key, echo the choice), a small **chip registry**, and
   a dormant `run_chips()` launcher.
...
**The missing half of the console (`src/011-cdc-acm.c`).** The USB serial link
could only *talk* — `debug_write` pushed bytes to your laptop, but the reverse
endpoint, though wired up, was never read. I added the listening half:
`console_getchar` arms a receive slot on that endpoint, waits for the controller
to DMA in whatever you typed, and hands the bytes back one at a time. Two
subtleties I had to get right — only one receive is ever in flight at once (a
timed-out wait leaves it armed and resumes, rather than starting a second one
the hardware would reject), and the "how many bytes arrived" count is read
through a volatile access because the controller scribbles it into memory behind
the compiler's back. There's a line-reader (`console_readline`) on top for when
a chip wants a whole typed word instead of a single key.


**The new category: chips (`src/020-chips.c`).** This is the interactive sibling
of the probe battery. The distinction, stated plainly: *probes are data, chips
are code.* A probe is a little declarative script swept at boot with a
machine-checkable verdict; a chip drives a device and asks a person "did you
feel/see/hear it?" — the only verdict a motor or a screen can give. The file
holds a reusable **menu** primitive (print numbered options, read a key, echo
it, branch — which is exactly the round-trip that proves I/O works), a small
**registry** of chips, and a launcher.


**The first chip: I/O device validation.** Its sub-menu has a console echo
self-test (type, watch it echo — this is the honest proof the read path works)
and a rumble-motor test that pulses the vibration motor weak then strong and
asks you to confirm each by feel.


**It's dormant, as you asked.** Nothing at boot calls the launcher — the whole
file compiles to *zero bytes* in a production image (I verified the object is
0/0/0), and there's a comment at the boot park marking the seam where a future
trigger (a button chord, a console command) would hand off to it.
...
### User Request 2
  ...
  rumble-motor test that pulses the vibration motor weak then strong and asks
  you to
...
how do we do typing if there's no keyboard attached? just button presses?
  ...
  against the confirmed register window, but the motor's PWM block still needs
  its
...
- **The keyboard is your laptop's keyboard.** You plug the handheld into your
  dev machine, open a serial terminal on the laptop (`picocom`/`screen
  /dev/ttyACM0`). What you type there rides down the USB cable as packets; the
  device's `console_getchar` receives them. The chip runs *on the device*, but
  it's listening to *your laptop*.

- **The screen is that same terminal window.** The menu is printed as text back
  up the cable; you read it on the laptop. Nothing is drawn on the device's LCD.
...
**Why this isn't throwaway work:** the menu logic reads an abstract "selection"
and writes abstract "lines." When the real screen (phase 111) and buttons (phase
5) land, you swap the output sink (serial → framebuffer) and the input source
(serial byte → button event), and the chip code is unchanged. And the serial
console *stays* even after the screen works — it's the dev-time interface you
use to test the screen and audio in the first place (you can't trust the panel
to report on the panel). "Get I/O working" meant this console specifically
because it's the one I/O surface already up; it's the bootstrap you build
everything else's tests on.
...
- **The pin.** The rumble pad's pinmux is `rockchip,pins = <0x03 0x14 0x01 …>`
  → **bank 3, pin 20 (= GPIO3_C4), mux function 1 = PWM**. So the mux write is
  GPIO3_C4's nibble set to 1, in the SYS_GRF pinmux register (GPIO1–4 live in
  SYS_GRF; GPIO0 is the PMU_GRF one the LEDs used). Exact offset is a one-line
  lookup in our own pinmux doc — the pin/bank/function is fully nailed.

- **The frequency.** The joypad wires the motor as `pwms = <pwm 0 100000 0>` →
  a **100,000 ns period = 10 kHz** drive. That's the number the period register
  targets.

- **The clocks.** `clock-names = "pwm","pclk"` → the **PWM3 function clock (id
  352)** and its **APB clock (id 351)**. Ungating them means clearing their bits
  in a `CLKGATE_CON` register, and un-resetting means a `SOFTRST_CON` bit —
  and those two register+bit pairs come from the **exact same TRM table we
  already used to find PWM1's gates** (`CLKGATE_CON31` bits 10/11). PWM3's entry
  sits a few rows over in that table.
...
### User Request 4
  ...
  you type there rides down the USB cable as packets; the device's
  console_getchar
...
just in case.
  ...
  writes abstract "lines." When the real screen (phase 111) and buttons (phase
  5)
  ...
  the screen and audio in the first place (you can't trust the panel to report
  on

  the panel). "Get I/O working" meant this console specifically because it's the
  one

  I/O surface already up; it's the bootstrap you build everything else's tests
  on.
...
right!! Good job!
  ...
  PWM3 confirmation probe (GPIO3_C4→func1, 10 kHz, plus the two
  CLKGATE/SOFTRST bits

  I'd pull from the TRM CRU table), you run a debug build and feel for it, and
  once

  confirmed the chip's rumble test inherits it. Want me to do that TRM lookup
  and
...
- **`G`**: `format=rgb888 lanes=4 flags=0xc03`, physical 81×61 mm, reset/sleep
  delays `20,10,50,120,50` ms

- **`M`**: `clock=42134` kHz, `horizontal=640,260,220,260`,
  `vertical=480,10,2,16` → **640×480, ~42.1 MHz, ~60 Hz**

- **`I seq=RRVV`**: the **complete DCS init sequence** as (register, value)
  pairs — starting with the JD9365 page-unlock `E0/E1/E2/E3` writes. No
  datasheet needed; it's right here.
...
- **DPHYs:** `0xFE850000` (DSI0) / `0xFE860000` (DSI1),
  `rockchip,rk3568-dsi-dphy`

- **Panel:** 640×480 @ 42.134 MHz, 4-lane RGB888, full DCS init sequence
  embedded, reset **GPIO0_B3** (bottom) / **GPIO0_B4** (top), active-low

- **Backlight seam (someone else's power question):** both `pwm-backlight`,
  brightness on PWM phandles `0xdf`/`0xe0` (25 µs period), gated by
  enable-GPIOs on controller `0x52` — the LED-boost voltage/current feeding
  those is the piece they're chasing
...
- **Backlight → docs/023** now records the real addresses
  (`0xFE700000`/`0xFE700010`, 40 kHz, gpio4 enable) and the key insight:
  brightness is PWM duty only, forward current is hardware-fixed, so there's no
  software path to burn it. I also noted the connection — those are channels 0
  and 1 of the *same* PWM controller whose channel 2 is the rumble motor, so one
  controller bring-up lights both backlights *and* buzzes the motor.

- **S8 → closed** in the safety doc, with the reasoning: both damage
  mechanisms are gone (can't over-drive the LEDs; panel timing is taken verbatim
  from the device tree, never synthesized). Marked structural, not a "remember
  to be careful."


**What I got done on the screen this turn:**

- **Captured the full display recon into `docs/023`** — the device tree
  resolved nearly every gap the doc flagged: the two DPHY bases
  (`0xFE850000`/`0xFE860000`), the panel timings (640×480, 42.134 MHz, 4-lane
  RGB888), the entire DCS init sequence, the reset GPIOs (GPIO0_B3 bottom / B4
  top), and the clock/reset IDs. Recorded as re-derivable from the DTB, not
  hard-coded truth.

- **Fixed a stale address** — issue 111a claimed VOP2 lived at `0xFEA00000`;
  it's `0xFE040000` (the version register answers there). Corrected, and updated
  the issue to say the controller's already proven alive.

- **Wrote the first real display driver — `src/021-vop2.c`** (VOP2 controller
  bring-up, the bottom of the stack). It ungates the clocks, releases the reset,
  verifies the version reads `0x40158023`, and confirms both video ports are
  parked in standby so nothing scans until framebuffers exist. Built clean in
  both variants; it's written and ready but deliberately *not* wired into boot
  yet, since a half-assembled display stack in the boot path is premature.


**Where the stack stands:** VOP2 (done, written) → DSI + DPHY (next) → panel
init (sequence in hand) → framebuffer + scanout → pixel. What still needs
real work before a panel lights: the **DPHY PLL divider math** (from the 42.134
MHz pixel clock up to the HS bit clock), the **CRU gate/reset bits** for the
DSI/DPHY clocks, the **GRF MIPI-mode/lane bits**, and confirming the **porch
field ordering** in the ROCKNIX timing string. Those are the 111b gaps — none
of them blocked anymore, just needing careful derivation.
...
- **MIPI** — Mobile Industry Processor Interface. The alliance that wrote the
  phone-display standards below; you'll see it prefixed on a lot of these.


**The data-path chain (RAM → glass)**

- **VOP2** — Video Output Processor, v2. Reads pixels out of RAM and streams
  them out.

- **VP0 / VP1** — Video Port 0 / 1. VOP2's two output taps, one per screen.

- **DSI** — Display Serial Interface. Packetizes the pixel stream to go down a
  skinny cable.

- **D-PHY** — the **PHY** is the *physical layer* (the analog circuitry that
  puts bits onto wires); the **D** is the Roman numeral 500, from its original
  ~500 Mbps target speed.

- **lane** — one differential wire-pair. "4 lanes" = 4 pairs carrying pixel
  bits in parallel.


**The two support blocks**

- **CRU** — Clock and Reset Unit. The power strip: hands out clocks and
  holds/releases each block's reset.

- **GRF** — General Register File. A junk drawer of system-wide mode switches
  (e.g. "this PHY is in MIPI mode," "use 4 lanes").

- **PLL** — Phase-Locked Loop. A circuit that multiplies a slow reference
  clock up to a fast one.


**The pixel & timing words**

- **RGB888** — 8 bits Red + 8 Green + 8 Blue = 24-bit color. **bpp** = bits
  per pixel.

- **pixel clock** — how fast pixels are pushed out (42.134 MHz ≈ 42M
  pixels/sec).

- **HS / LP** — High Speed / Low Power. DSI's two gears: LP (slow) to *tell
  the panel things*, HS (fast) to *stream pixels*.

- **DCS** — Display Command Set. The standard "sleep out / display on"
  commands (plus vendor register pokes) — that's the init sequence.

- **porch / sync** — blanking gaps around each line and frame. Leftover
  vocabulary from CRT TVs, where the electron beam needed idle time to fly back;
  digitally they're still padding the panel expects. **H**/**V** = horizontal
  (per line) / vertical (per frame); **FP/BP/SA** = Front Porch / Back Porch /
  Sync Active.

- **ROCKNIX** — not an acronym; it's the name of the handheld-Linux firmware
  we pulled the device tree from.
...
- **CRU** has to have switched each block's clock on and released its reset —
  otherwise VOP2/DSI/D-PHY are dark and unresponsive (this is the part already
  confirmed on for VOP2).

- **GRF** has to be set so the D-PHY knows it's speaking MIPI over 4 lanes, not
  something else.

- The D-PHY's **PLL** has to be programmed to produce the right **HS**
  bit-clock. That's the "divider math": from 42.134M pixels/sec × 24 bits ÷ 4
  lanes you get the target lane speed, and the PLL needs the multiply/divide
  numbers that land on it.
...
forward?
  ...
  So the 111b "gaps" in plain terms: I need the PLL's multiply/divide numbers
  (PLL math), the

  specific CRU switches for DSI/D-PHY, the GRF mode bits, and to confirm which
  of the four timing
...
- **Agent 1 — the CRU switches.** CRU = Clock and Reset Unit. It's finding the
  exact register + bit that turns on the clock and releases the reset for each
  DSI controller and each D-PHY, by mapping the device-tree clock IDs to the
  CRU's gate/reset tables.

- **Agent 2 — the GRF bits, the DSI wake-up order, and the porches.** GRF =
  General Register File (the junk-drawer of mode switches). It's finding which
  GRF bits put each D-PHY in MIPI mode at 4 lanes and route each Video Port to
  its DSI; confirming the order of register writes that walks the DSI host into
  command mode (the slow gear used to send the panel its init sequence); and
  pinning down which of the four timing numbers is front-porch vs back-porch vs
  sync.

- **Agent 3 — the PLL math.** PLL = Phase-Locked Loop (the clock multiplier).
  It's extracting Rockchip's D-PHY PLL algorithm and working out the actual
  multiply/divide numbers to turn the 42.134 MHz pixel clock into the ~253
  Mbps-per-lane high-speed bit clock, plus the register sequence to lock the PLL
  and light up the four lanes.
...
- **Upstream driver (network):** `curl` is denied by the Bash permission hook
  (both sandboxed and with `dangerouslyDisableSandbox`), and the `WebFetch` tool
  is denied. No network path reaches `raw.githubusercontent.com`. The driver was
  therefore **NOT fetched and NOT saved** to
  `docs/driver-refs/phy-rockchip-inno-dsidphy.c` — I will not fabricate a
  "source" file. To unblock, run yourself:
...
- **TRM Part 2 Ch30 (PDF):**
  `/mnt/mtwo/programming/ai-stuff/soren-ds/docs/datasheets/rk3568-trm-part2.pdf`
  exists (125 MB). The **Read tool refuses PDFs &gt;100 MB** ("exceeds maximum
  allowed size"), the `gs` binary is **fully blocklisted** by the Bash hook
  (even `gs --version` is denied), and `pdftotext`/`pdfinfo`/`mutool` are **not
  installed**. There is no available path to extract Ch30 text. To unblock:
  install `poppler-utils` (`pdftotext`), or let me run `gs` (whitelist it), or
  split Ch30 into a &lt;100 MB PDF I can Read.


**What I *could* use instead:** the repo's own `docs/023-display-controller.md`,
which is prior recon that cites TRM Part 2 Ch30 and already carries the
ANALOG_REG map — and my recall of the `phy-rockchip-inno-dsidphy.c` driver
family. **Every value below is tagged** `[repo-023]` (in-repo, cites TRM),
`[recall]` (driver-family knowledge, unverified this session), or `[UNRESOLVED]`
(needs the blocked sources).
...
- **Raw minimum** (`[repo-023]`, DTB-confirmed panel: 42.134 MHz, RGB888, 4
  lanes):

  `total = 42.134 MHz × 24 bpp = 1011.216 Mbps`; `per-lane = 1011.216 / 4 =
  252.804 Mbps/lane`.

- **What the Rockchip DSI driver actually requests** `[recall]`:
  `dw_mipi_dsi_get_lane_mbps()` in `dw-mipi-dsi-rockchip.c` multiplies by **10/8
  (×1.25)** — the comment is literally *"since mbps must be bigger than the
  bandwidth of RGB"* — and rounds the pixel clock up to whole MHz first:
...
**This 25% margin *is* the "minimum lane-rate clamp" the prompt suspected** —
the PHY is programmed for **~322 Mbps/lane (~1288 Mbps total), not 252.8.**
252.8 &gt; any absolute floor (~80–90 Mbps), so no *further* clamp fires.
Whether your kernel path uses this old per-driver function or the generic
`phy_mipi_dphy_config` (which passes lane rate without the ×10/8) is the one
thing to confirm against the fetched driver — but the ×10/8 margin is
long-standing for Rockchip.
...
F_vco = F_ref × FBDIV / PREDIV        (VCO constrained to a band, ~80–1500 MHz class)
...
- `F_ref = 24 MHz`; `PREDIV` (ANALOG_REG03), `FBDIV` (ANALOG_REG04), `POST`
  (ANALOG_REG08) per `[repo-023 lines 112-113]`.

- **[UNRESOLVED]** the exact DDR factor placement — whether the driver's
  "rate" variable is the lane bit rate or the DDR clock (= lane rate ÷ 2). This
  ×2 ambiguity changes the concrete FBDIV by 2×, so I cannot pin the single
  correct integer set without the file.

- **[UNRESOLVED]** exact PREDIV/FBDIV/POST bitfield positions and byte values,
  the VCO min/max, and the input-divider band constants.
...
- **If "rate" = lane bit rate directly** (POST=1): `FBDIV/PREDIV = 320/24 =
  13.33`.
...
**My best single answer, flagged:** program for ~320 Mbps/lane with **PREDIV=3,
FBDIV=40, POST=/1** *if* the register holds the lane rate directly, or
**PREDIV=3, FBDIV=20** *if* it holds the DDR clock. Verify against the fetched
`inno_dsidphy_pll_calc_rate()`.
...
| 1 | `+0xE3` LVDS_REG03 | mode = MIPI | Select MIPI (not LVDS/TTL) mode on this combo PHY | `[repo-023]` / value `[UNRESOLVED]` |

| 2 | `+0x03` ANALOG_REG03 | PREDIV (=3?) | PLL input divider | `[repo-023]` / encoding `[UNRESOLVED]` |

| 3 | `+0x04` ANALOG_REG04 | FBDIV (=40 or 20) | PLL feedback multiplier | `[repo-023]` / encoding `[UNRESOLVED]` |

| 4 | `+0x08` ANALOG_REG08 | POST (=/1) | PLL post/rate divider | `[repo-023]` / encoding `[UNRESOLVED]` |

| 5 | `+0x01` ANALOG_REG01 | `0xE3` | PLL power-up / bandgap enable | `[repo-023]` |

| 6 | `+0x00` ANALOG_REG00 | `0x01` + lane bits | Power + enable clock lane &amp; 4 data lanes | `[repo-023]` / exact lane-enable bits `[UNRESOLVED]` |

| 7 | `+0x20` DIGITAL_REG00 | `0x1F` | Release digital reset / enable digital block | `[repo-023]` |

| 8 | poll lock | see below | Wait for PLL lock | see below |


**PLL-lock poll:** the reliable, well-defined bit is on the **DSI host** side,
not the analog PHY — `MIPI_DSI_HOST` base + `0xB0` `PHY_STATUS`, `phy_lock`
bit (DW MIPI DSI v1.31) `[repo-023 line 89]`. Release order on the host:
`PHY_RSTZ` (host `+0xA0`: `phy_shutdownz`, `phy_rstz`, `phy_enableclk`, then
`phy_forcepll`), set `PHY_IF_CFG` (`+0xA4`) `n_lanes = 3` for 4 lanes, then poll
`PHY_STATUS.phy_lock` `[repo-023 lines 86-90]`. The **inno PHY's own internal
lock-status register/bit is [UNRESOLVED]**.
...
- **Solid:** 24 MHz ref; lane rate = pixel×bpp/lanes = 252.8 Mbps/lane raw; the
  ×10/8 → ~322 Mbps/lane driver margin; integer-feedback PLL
  `F_ref×FBDIV/(PREDIV×2^POST)`; the ANALOG_REG offset/reset map; host-side
  `PHY_STATUS.phy_lock` as the lock poll.

- **UNRESOLVED (needs the two blocked sources):** the DDR ×2 placement →
  exact FBDIV integer; the PREDIV/FBDIV/POST bitfield encodings and byte values;
  register offset stride (byte vs &lt;&lt;2); the inno PHY's own lock bit;
  VCO/input-divider band constants; and the exact GRF routing offset.
...
**That's the unblock.** `ghostscript` (10.06.0) runs fine for me, plus `qpdf`
and `pdftk` — the sub-agents were denied Bash, but I'm not. So I can extract
the TRM Part 2 DSI/D-PHY chapters myself and get the authoritative D-PHY PLL
register encodings that Agent 3 had to leave unresolved. No guessing needed
after all.
...
PLL_OUTPUT = (FREF / PREDIV × FBDIV) / POSTDIV,  FREF = 24 MHz,  PLL_OUTPUT = lane bit rate
...
- **CRU:** DSI host `pclk` = `GATE_CON21` (0xFDD20354) bits 6/7; D-PHY `pclk` =
  `GATE_CON33` (0xFDD20384) bits 14/15; DSI reset = `SOFTRST_CON17` (0xFDD20444)
  bits 0/1; D-PHY reset = `SOFTRST_CON27` (0xFDD2046C) bits 11/12. Agent 1's
  WebSearch values confirmed exactly.

- **D-PHY:** full register map, the MIPI init sequence (TRM §30.4.1), and the
  PLL formula (§30.4.3) — all verified.
...
- **The CRU switches** (Clock and Reset Unit) — **TRM-verified, all four.**
  DSI-host clock in gate-register 21, D-PHY clock in gate-register 33, their
  resets in reset-registers 17 and 27, exact bits confirmed against the manual's
  own bit tables. (Agent 1's from-memory values matched exactly — nice
  cross-check.)

- **The GRF bits** (General Register File) — **turns out none are needed.**
  The manual's D-PHY init sequence shows MIPI mode is the *power-on default*;
  the GRF lane-control registers are only for LVDS (a different panel type). One
  less thing to get wrong.

- **The PLL math** (Phase-Locked Loop) — the manual handed me both the formula
  *and* a worked example init sequence with real register values. Output
  frequency = 24 MHz ÷ prediv × fbdiv ÷ postdiv, and that output *is* the
  lane bit rate. For our panel I computed prediv 2, fbdiv 27 → ~324 Mbps per
  lane (matching the ×1.25 headroom Rockchip's driver always adds over raw
  pixel bandwidth).

- **The porch order** — deferred, because it's genuinely not needed yet.
  Porches only matter once the DSI switches to *video* mode (streaming pixels);
  this bring-up stops at *command* mode (ready to send the panel its wake-up).
  Best determination stands at ~70% (sync is the middle number), to confirm
  later.
...
**`src/022-mipi-dsi.c`** — brings up both screens' DSI controller + D-PHY to
command mode with the PLL locked, one function run twice (bottom via Video Port
0, top via Video Port 1). Built clean in both variants, dormant/ready like the
VOP2 driver — it gets wired into boot as a set once the panel init and scanout
land. Every register value carries its TRM citation in a comment.


**Three residuals I flagged loudly** (TRM-derived, but hardware-unverifiable
from here, so honest about it):

1. the D-PHY register **stride** (`index<<2` — the manual lists byte indices
   behind an internal address-mapping note; this is the standard Rockchip
   pattern but the one thing not stated outright),
...
### User Request 8
  ...
  command FIFO — then it's just replaying the ~200 commands the DTB already
  gave us.
...
**Is it a probe or `main.c`-style driver code?** Driver code — same category
as the VOP2 and DSI bring-ups I just wrote, *not* a probe. Probes are the little
declarative fire-and-log scripts (`W`/`R`/`DELAY`) run by the probe engine; the
panel wake-up is real C with a loop and a FIFO handshake, so it's a
`panel_init()` function in the driver layer, not a probe. Mechanically it's
small: one helper that packs a 2-byte command (register + value) into a **DCS**
(Display Command Set — the panel's command language) packet and pushes it
through the DSI host's command **FIFO** (first-in-first-out queue), then a loop
that replays the ~200 commands the device tree already handed us, plus the
"sleep out" and "display on" at the end.


**Every boot, or dormant?** Dormant — nothing wired to run yet. Exactly like
`vop2_init` and `mipi_dsi_init`: written, compiled, and parked. The whole
display chain gets switched on as one unit once scanout (111d) exists, because a
half-built display path lit at boot would just narrate into the void — there's
nothing to *see* until pixels scan and the backlight's on.
...
- **The LED is the prompt** — a distinct blink pattern (nothing else uses)
  says "capture window open, press now."

- **The SD log is the record** — the device samples the input hardware and
  writes down what it sees.

- **Your spoken press-order is the labels** — you press A, B, X, Y, D-pad…
  in an order you tell me, and afterward we line up "reading changed *here*"
  against "you said you pressed *that*." That correlation *is* the button map.
...
**Am I already working on it?** Honestly, no — I've been heads-down on the
display. But it's the right next thing for input, and I'm glad to build it.


So, your call on order (most useful first):

1. **Pivot to input now** — I crack the v2 SARADC from the manual, then build
   the LED-signaled capture tool; you do a button-pressing pass and we get a
   real map. This is the one that produces hardware results immediately.
...
### User Request 9
  ...
  that turns a voltage into a number). And right now we can't read that
  converter: the existing

  gamepad probe drives it with the v1 register layout, but this chip is a v2
  part, so every

  channel reads a constant 0x3FF — garbage. Only the volume buttons and the
  hall sensor are plain
  ...
  exactly the way I cracked the D-PHY: the old probe's author believed the
  manual only documents
...
- **Reset pulse:** GPIO0 (`0xFDD60000`), `DDR_L`@0x08 + `DR_L`@0x00,
  write-masked, pin 11 (bottom) / 12 (top), active-low

- **Command send:** poll `gen_cmd_full`, write `GEN_HDR = 0x15 | reg<<8 |
  val<<16` (DCS short write, 1 param)

- **The 203 commands:** extracted from the device tree, one per-panel override
  (register 0x37)
...
**Confirmed: the TRM only documents the v1 SAR-ADC** — `DATA`@0x00,
`STAS`@0x04, `CTRL`@0x08 — exactly the layout the broken probe already uses.
The v2 registers (`CONV_CON`/`STATUS`/`DATA`@0x120) that this chip actually
needs are **not in the manual** — only in the Linux driver. This is the wall I
promised I'd flag plainly. Before I do, let me check whether that driver (or any
kernel source) happens to be on this machine already:
...
**`src/023-mipi-panel.c`** wakes both panels: pulse the reset line (GPIO0,
verified registers), set the DSI host to **low-power command mode**, replay the
JD9365's 203-command **DCS** (Display Command Set) init table, then Sleep-Out
and Display-On. The 203 commands were **pulled straight from the device tree
with a script**, not hand-typed — the two panels turned out identical except a
single byte (register 0x37), handled as a per-panel override. Everything's
TRM-verified except one flagged choice (sending the commands as DCS vs generic
writes — a one-constant swap if wrong). I deliberately **stopped before the
command→video-mode switch** because that needs the panel's porch timing, which
I haven't confirmed — so rather than guess timing, I left that for 111d. Both
builds clean.
...
1. **Get me that one file.** `drivers/iio/adc/rockchip_saradc.c` from mainline
   Linux (it's small). If your interactive shell has network the sub-shells
   don't, `! curl -fsSL -o docs/driver-refs/rockchip_saradc.c
   https://raw.githubusercontent.com/torvalds/linux/master/drivers/iio/adc/rockchip_saradc.c`
   would drop it in; otherwise grab it on any machine and drop it there, or
   point me at a kernel tree if you have one somewhere my timed-out search
   missed. Then I do the v2 fix correctly.
...
### User Request 10
  ...
  drop it in; otherwise grab it on any machine and drop it there, or point me at
  a kernel tree if
...
- **Next flash:** SARADC v2 discovery runs (above). You report the log + stick
  movements.

- **After that:** with the layout confirmed, I finish the v2 reader, then build
  the LED-signaled **input-capture tool** — now covering the analog half (face
  buttons, D-pad, sticks) *and* the GPIO half (volume, hall) — which you
  slotted for "after the next flash."

- **Parked, untouched:** the whole display chain (VOP2 → DSI/D-PHY → panel
  init) stays dormant and out of the boot path — no half-built display lit at
  boot to risk a hang, so your discovery flash stays clean.
...
**The image that ran was stale** — built *before* my v2 rewrite. My unique
strings ("discovery", "CONV_CON", "RECALLED") aren't in it at all, and the old
probe was still de-selected (`#NEEDED 0`), so the SARADC discovery never ran and
no ADC data was captured. Your stick-wiggling had nothing listening. Let me
confirm whether the *current* build has my probe and whether it made it onto the
carry drive:
...
- **The slow probe you saw isn't a hang** — it's a legitimately heavy one (the
  eMMC scan walks the whole chip). The SARADC wasn't even it; that one was
  skipped this run. Nothing to worry about.

- **This desync will bite again** unless we wire it shut. Want me to make
  `scripts/build` run `build-bootable-sd` automatically after a successful
  `--debug` build? Then `kernel.img` and `bootable-sd.img` can never drift
  apart, and "I flashed but it ran old code" stops being possible. Small change,
  saves a flash cycle every time.
...
### User Request 12
  ...
  bootable-sd.img can never drift apart, and "I flashed but it ran old code"
...
1. `+0x0C = 0x08` is the **documented v1 `DLY_PU_SOC` reset value**. And
   `0x00/0x04/0x08 = 0` are v1 `DATA`/`STAS`/`CTRL`. So the **v1 registers are
   real and present**.
...
2. The window **mirrors that 16-byte block** everywhere — `0x120` reads the
   ...
   registers** at `0x104`/`0x110`/`0x120` (which is why my v2 conversion timed
   out and read 0). The chip decodes only 4 address bits here.
...
- **releases the SARADC reset** (`presetn_saradc`) — which the old probe never
  did

- powers the converter **down first**, then selects the channel and enables the
  end-of-conversion flag, then powers **up** to start — in separate writes, as
  the manual requires
...
**Hold the left stick fully into one corner** through the run. That way, if a
channel is one of that stick's axes, it'll read near `0x000` or `0x3FF` while a
centered axis sits near `0x200` — the spread across channels tells us which is
which. (You don't need to time anything — every channel gets read in one quick
sweep.)
...
- **Values that vary by channel and move with the stick** → the ADC reads
  work, and we can start mapping axes and the button ladder. Straight into the
  input-capture tool from there.

- **Still a flat `0x3FF` everywhere** → the digital side is fine but the
  analog front-end (the ADC's reference/supply) isn't powered — that becomes
  the next thing to chase (likely a PMIC regulator), and I'll know exactly where
  to look.
...
**It works.** Look at the results — six *different, real* values instead of
the old flat `0x3FF`:
...
- **Channel 3 = `0x20D`** is the only joypad input in the list — one stick
  axis, at whatever position the mux defaulted to (reading near-centered).

- **Channels 0, 1, 2, 4, 5** aren't joypad at all — they're other board
  signals (battery sense, detect lines) or unconnected pins floating high.
  That's noise, not sticks.
...
### User Request 14
  ...
  - Channel 3 = 0x20D is the only joypad input in the list — one stick axis,
...
I was moving the left joystick the entire time
  ...
  positions, reading channel 3 each time — that captures all four stick axes
  in one flash. Same

  probe can dump the button GPIO ports so we see every digital button's state
  too. One flash then
...
**On the joystick** — you moving the left stick the whole time and channel 3
*still* reading centered isn't a contradiction, it's the proof: the mux was
feeding us a *different, unmoved* axis. You can't even tell which axis you're
measuring without flipping the mux and cross-referencing against a known
movement. That's the tedium you're describing, baked right in. Every axis and
every button would be a flash-and-decode round trip. Not worth it when the goal
is just "which pin is the A button."


**The strategic call is sound.** Grinding input through logs+LEDs is the slow
path. Let me weigh your two alternatives:


- **USB flashing** — yes, this would make input trivial (a live interactive
  console). But it's the bring-up that's *not* up and going slowly, and it's a
  genuinely hard/independent effort — not something I can quickly unblock for
  you. Betting the input work on it means waiting on the thing that's already
  stuck.

- **The screen** — we're most of the way there already. VOP2, the DSI + D-PHY
  link, and the panel init sequence are **all written**. What's left is the
  framebuffer + scanout, the backlight, and wiring the chain into boot. Once a
  panel lights, a live on-screen input viewer is easy — press a button, watch
  it highlight; nudge a stick, watch a bar move. No decode round-trips.


**My recommendation: finish the screen.** It's the closer of the two, it's ours
to drive (not blocked on someone else), a lit panel is a real milestone on its
own, and it's exactly the substrate your input viewer wants.
...
1. **111d** — allocate the framebuffers and configure VOP2 to scan them out,
   plus the command→video-mode switch (this is where the panel *porch timing*
   I flagged finally has to be pinned — I'll extract it and the last VOP2
   scanout bits from the manual, same as I've been doing).

2. **Backlight** — bring up its PWM (channels 0/1 of the *same* controller as
   the rumble, so resolving that controller's clock/reset/mux serves both).
   Without it the panel scans but stays dark.
...
- **`REG_CFG_DONE` @ 0x0000** — hiword-masked (bits 31:16 write-enable), bit
  15 `sw_global_regdone_en`, per-layer load bits (esmart0 = bit 10).

- **`DSP_INFACE_EN` @ 0x0028** — `mipi_out_en` = bit 4 with VP-mux at 17:16;
  `mipi1_out_en` = bit 20 with VP-mux at 22:21. So VP0→MIPI0 + VP1→MIPI1 =
  `0x00310010`.
...
**Multi-page allocator — done and build-verified.** The display framebuffers
can't be scattered pages: VOP2 reads each one as a single linear span from one
base-address register, so it needs ~300 *contiguous* pages (640×480×4 bytes).
I added `alloc_pages(N)`/`free_pages()` to the allocator — a sliding-window
scan over the page bitmap for a run of N free pages — and extended the boot
self-test to prove a run comes back page-aligned, adjacent, and reusable after
freeing. Both builds clean.


**VOP2 registers — mapped from the manual.** VOP2 is the most involved block
in the whole stack, so I'm reading it carefully rather than guessing (you've
been clear on that). What I've pinned down from TRM Ch13:

- **Commit** — `REG_CFG_DONE` @ 0x00 (hiword-masked, global-enable bit 15 +
  per-module load bits).

- **Interface enable** — `DSP_INFACE_EN` @ 0x28: turn on MIPI0 from VP0 and
  MIPI1 from VP1 → `0x00310010`.

- **Output mode** — `dsp_out_mode = 0` for 24-bit RGB888.

- **VP timing** — the per-port `DSP_HTOTAL_HS_END`@0x48 /
  `DSP_HACT_ST_END`@0x4C / `DSP_VTOTAL_VS_END`@0x50 / `DSP_VACT_ST_END`@0x54
  registers, plus the standby bit (31) that gates scanout.

- **Overlay routing** — `LAYER_SEL`@0x04 / `PORT_SEL`@0x08 (which layer feeds
  which port).


**Still to read** (the last dig before I write the scanout driver): the
window-layer's exact framebuffer-pointer/stride/format/size bit-fields, the
precise `PORT_SEL` routing values, and the per-VP commit bits. Then it's:
...
- **`vop2_scanout()`** — the whole VOP2 scanout path, every register read
  straight from the manual: point the Esmart layer at a framebuffer (ARGB8888,
  full-screen), program the VP timing, route the port to its MIPI panel, leave
  standby, and commit. Three residuals are flagged in-code (the porch field
  order at ~80%, the overlay left at its Esmart→VP reset routing, and the
  sub-4GiB framebuffer constraint) — the honest "here's what to poke if it's
  dark" markers.
...
2. **Display orchestrator** — allocate the two framebuffers, paint a test
   pattern (so first light shows something unmistakable, like colour bars), and
   run the sequence: VOP2 init → DSI init → panel init → video mode →
   scanout.
...
- **VOP2** clocked → **DSI + D-PHY** up → **panel** DCS-initialized →
  **video mode** → **scanout** of two 640×480 framebuffers painted with
  **colour bars** → **backlight on** (fully resolved this turn: clock, reset,
  pad mux GPIO4_C5/C6→PWM, duty, enable GPIOs).

- Runs after the probe sweep in the debug boot path, so you still get the probe
  logs even if a display step stalls.
...
| **Colour bars on both screens** | 🎉 the whole stack works (colours also reveal RGB vs BGR order) |

| **Lit but blank/garbage** | scanout side — porch order, overlay routing, or the VOP2 IOMMU |

| **Image rolls or shifts** | porch field order (~80% guess) — a one-line swap |

| **Backlit but black** | panel init or DSI video timing (the lane-rate/lbcc factor) |
...
**Both D-PHY PLLs locked**, with lane stop-state — that validates the entire
DSI/D-PHY derivation (the TRM-extracted PLL sequence, the `index<<2` register
stride, the CRU clocks — the part with the *most* flagged uncertainty). Then
both panels took their full init sequence + sleep-out/display-on without
stalling, both hosts switched to video mode, both VOP2 ports configured
(framebuffers at `0x0206B000`/`0x02197000`, comfortably under 4 GiB), backlight
on, "both screens scanning."
...
*Q1 — Screen output:* The bring-up log is all green (both D-PHY PLLs locked,
every stage ran). What do the two panels actually show? If the two screens
differ, pick the more interesting one and tell me the difference.

- Colour bars — One or both show vertical colour bars — even if the colours
  look wrong, or it's shifted/rolling/tinted. This is the win case.

- Lit but garbage — Backlit and showing SOMETHING, but not clean bars —
  noise, blocks, static, or a scrambled/repeating image.

- Glowing but black — The backlight is clearly on (panel glows / you can see
  it's lit in a dark room) but the image is solid black.

- Dark / nothing — No visible backlight and no image — the panels look
  completely off.
...
*Q1 — Screen output:* What do the two panels actually show? If the two screens
differ, pick the more interesting one and tell me the difference in the notes.
...
- Glowing but black — The backlight is clearly on (panel glows / visible as
  lit in a dark room) but the image is solid black.
...
**The likely fix:** the test pattern is now **opaque** (`0xFF` alpha). My bars
were `0x00RRGGBB` — alpha 0 — and an ARGB8888 layer blends that as fully
transparent, so the bars were invisible over the black background. That single
byte per pixel would produce exactly the "backlit but black" you saw.


**Two diagnostics, so this flash is decisive whichever way it goes:**

- A **green VP background**. If the bars still don't appear, a *green* screen
  proves VOP2 is genuinely scanning valid video to the panel — which would
  isolate the problem to the layer/framebuffer fetch (window enable, or the
  IOMMU) rather than a dead scanout. (Opaque bars cover it, so green only shows
  on failure.)

- A **register read-back** logged as `[vop2] rb DSP_CTRL=… ESMART_CTRL=…
  INFACE=…` — tells me whether standby actually cleared (bit 31 = 0) and the
  window enabled (bit 0 = 1), i.e. whether the commit latched.
...
| **Solid green** | VOP2 scans fine; the framebuffer layer isn't reaching output → I chase the window fetch / IOMMU (the `rb` line will show enable state) |

| **Still black** | scanout itself isn't running → the `rb DSP_CTRL` value tells me if standby stuck / commit didn't latch |
...
**⚠️ Worth a look:** `emmc-wipe` is armed and runs on every debug boot. If
that's not intentional (the parallel person's, and they know), you'll want it
off too — say the word and I'll disable it.
...
**Next:** I'll dig into the CRU to bring `DCLK_VP0/1` up at the ~42.134 MHz
pixel rate (it comes off a PLL through dividers, like the D-PHY clock did) and
wire it into `vop2_init`. That's the most likely single thing standing between
us and a lit image. Want me to go after it?
...
- **`display-recon`** asks: *is u-boot/ROCKNIX already scanning out to the panel
  at boot?* (reads the POST standby bits, interface-enable, and the Esmart
  framebuffer pointer).

- **`backlight-duty`** reads the live backlight PWM state — and confirms my
  backlight approach exactly (PWM3 `0xFE700000` ch0/ch1, DUTY/PERIOD brightness,
  gpio4 enables). No conflict there; good cross-check.
...
**⚠️ Before you flash — `emmc-wipe` is the one probe still armed, and it
will run this boot.** If it does what the name says, it wipes the eMMC every
debug boot. I left it per your "leave what I don't recognize" rule, but that's a
loud thing to leave running — want me to de-select it too? One word and it's
off.
...
| **POST top bit = 0** (e.g. `0x0…`) **and Esmart0_fb ≠ 0** | u-boot is already scanning → DCLK/clocks are up. My re-init is *disrupting* a working panel. I stop cold-starting and instead preserve firmware's clock setup. |

| **POST = `0x8…`** (standby) **and fb = 0** | u-boot left it idle → I really am missing the pixel clock, and I set up DCLK from the CRU. |
