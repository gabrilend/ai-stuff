...
The key finding: commit f63c0f2b says the similarity **and diversity caches
...
- word clouds now render in real color, gray reserved for the faint (`f2b197a2`)

- source browser links out to originals instead of hosting copies + a download-everything button (`156e9700`, `3c5c2323`)

- collision-safe relative image links across galleries (`26167aae`)

- word-page links pointed up to the site root (`2d69865a`)

- the explore-math page gained a word-cloud-placement explanation (`160e9af7`)

- per-poem page ceiling now measured from your storage quota (`514076d4`)
...
lists** in `config.lua` (`07fedb78`, `2a5bc9f3`, `b33b46aa`, `5410d889`): those
...
`7c48268e`), so this explanation text may be stale |
...
**The diversity "shuffle" doc is stale.** `flat-html-generator.lua:3228-3231` tells users on the explore-math page that the diversity walk "is shuffled first with a Fisher–Yates pass… re-seeded from the clock on each build." That described the **CPU diversity engine you deleted yesterday** (`7c48268e`). The GPU replacement is deterministic — no shuffle, no clock. So that page is now lying to readers. Per your own rule ("find the documentation that caused it and fix the docs"), this text should be corrected as part of this work.
...
yesterday (7c48268e). The GPU replacement is deterministic — no shuffle, no
