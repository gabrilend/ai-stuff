...
- Honest caveat: this session has been enormous, so I can't be 100% certain *every* hunk's authorship from memory alone. The media-path refactor *looks* clearly not-mine (wrong functions, a theme that matches an already-committed `6c2f58d0 "make images show up where they actually ship"`), but you'd know for sure. That uncertainty is exactly why splitting blind is risky here.
...
- **Commits made**: 156e9700 (source browser), 160e9af7 (explore math).
...
Two commits already landed: 156e9700 (source browser), 160e9af7 (explore math).
...
Commit 5 landed (`b33b46aa`). Now I'll work through the remaining task-commits.
...
| `b33b46aa` | Every image/sync source mandatory (tore out the "optional"
concept) |

| `d4fba6ac` | Word-cloud "all words" routed through one setting |

| `f2b197a2` | Word-cloud real color + gray-for-faint + archive keepsakes +
palette fingerprint |

| `e700dbd9` | New memory-budgeter library |

| `f63c0f2b` | Fix B — neighbour caches capped to top-K |

| `514076d4` | Per-poem page ceiling computed from the storage quota |

| `7c48268e` | GPU made the only route (CPU engines deleted, −4764 lines) |

| `16358ac4` | Diversity shader/lib paths unified with similarity |

| `11bc6e3d` | Budgeter wired into HTML (RAM) and diversity (VRAM) |
