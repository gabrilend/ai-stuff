...
-- These tests build their OWN similarity fixture rather than reading the
-- project's real one, and that is the point of the change that made them run
-- again.
--
-- They used to open a single similarity_matrix.json under a model folder named
-- "embeddinggemma_latest". The folder was renamed long ago and the single-file
-- format was retired in Issue 8-033 -- similarity now lives as one file per poem
-- under similarities/. So all four tests failed on a missing file and the suite
-- concluded "algorithm needs review", which was wrong twice over: the algorithm
-- was never exercised, and the thing that had changed was storage, not logic.
--
-- A test of an algorithm should not depend on the output of a five-hour pipeline
-- stage. A small hand-built fixture exercises the same code paths, runs in
-- milliseconds, and cannot be broken by a storage change somewhere else.
...
-- {{{ local function build_similarity_fixture
-- Write a small similarity file in the nested-map shape diversity-chaining reads:
-- { similarities = { ["1"] = { ["2"] = score, ... }, ... } }.
--
-- Scores are deterministic and deliberately spread, so a chain that picks the
-- LEAST similar next poem has a clear right answer to find. Symmetric, because
-- the real store is bidirectional.
local function build_similarity_fixture(path, poem_count)
    local sims = {}
    for a = 1, poem_count do
        sims[tostring(a)] = {}
    end
    for a = 1, poem_count do
        for b = 1, poem_count do
            if a ~= b then
                -- Poems near each other in index are similar; far apart, not.
                -- A simple, checkable relationship rather than random noise.
                local distance = math.abs(a - b) / poem_count
                sims[tostring(a)][tostring(b)] = 1.0 - distance
            end
        end
    end
    utils.write_json_file(path, { similarities = sims })
    return path
end
-- }}}
...
local function run_basic_tests()
    ...
    local similarity_file = build_similarity_fixture(
        DIR .. "/tmp/diversity-chaining-test-similarity.json", 24)
...
local function main()
    ...
    utils.log_info("\n" .. string.rep("=", 50))
    -- The fixture is regenerated by every run; remove it so a stale copy can
    -- never be mistaken for real project data sitting in tmp/.
    os.remove(DIR .. "/tmp/diversity-chaining-test-similarity.json")
