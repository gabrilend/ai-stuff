...
### Step 1: Update `scripts/generate-html-parallel`
...
elseif a:match("^--pages=") then
    opts.pages = tonumber(a:match("^--pages=(%d+)$"))
elseif a:match("^--poems%-per%-page=") then
    opts.poems_per_page = tonumber(a:match("^--poems%-per%-page=(%d+)$"))
```

Update help text:
```
  --pages=N            Number of pages per poem (default: from config, 15)
  --poems-per-page=N   Poems per page (default: from config, 100)
```

Pass options to generation functions or override config values before generation.

### Step 2: Update `src/flat-html-generator.lua`

Add function to accept runtime overrides:
...
Or modify `load_pagination_config()` to accept optional overrides parameter.

### Step 3: Update `run.sh` TUI Menu

Add pagination options to the TUI menu for Stage 9 (HTML Generation):
...
- [ ] `--pages=N` flag works in `generate-html-parallel`
- [ ] `--poems-per-page=N` flag works in `generate-html-parallel`
- [ ] Flags override config file values
- [ ] `run.sh` TUI includes pagination options
