...
1. **run.sh**:
   - Added `WORDCLOUD_POEMS=""` variable
   - Added `--wordcloud-poems N` CLI flag parsing
   - Added TUI menu item in "Word Cloud Options" section with `p` hotkey
   - Passes `--poems-per-page $WORDCLOUD_POEMS` to generate-word-pages.lua

2. **src/generate-word-pages.lua**:
   - Updated `parse_args()` to handle `--poems-per-page` flag
   - Stores value in `CLI_POEMS_PER_PAGE` module-level variable
   - Updated CONFIG construction to use precedence: CLI > config.lua > default (50)

### Configuration Precedence

```
CLI --wordcloud-poems N > config.lua word_cloud.poems_per_page > default (50)
```
...
```bash
# Via CLI
./run.sh -H --wordcloud-poems 25

# Via TUI
# Navigate to Word Cloud Options, press 'p', enter value
```

### Note on Naming

The CLI flag is `--wordcloud-poems` (run.sh side) which passes as `--poems-per-page` (Lua side). This maintains consistency with existing patterns while keeping the external interface clear.
