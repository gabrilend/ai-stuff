...
1. **Added helper functions** (lines 391-451):
   - `compute_centroid()` - Averages embeddings of selected poems
   - `find_closest_poem_to_centroid()` - Finds poem nearest to centroid via cosine similarity

2. **Built chronological page map** (lines 934-937):
   - Extended existing chrono_map construction to also build `chrono_page_map`
   - Maps poem_index → page string ("index", "02", "03", etc.)
   - Uses `unified_config.pagination.chrono_per_page` (default 500)

3. **Updated `generate_word_page()` function** (lines 762, 778-780, 794):
   - Added `chrono_center_link` parameter
   - Updated header to include Main, Word Cloud, and centroid-based Chronological links

4. **Updated main generation loop** (lines 1074-1095):
   - Computes centroid of selected poems after balanced color selection
   - Finds closest poem to centroid in entire corpus
   - Builds anchor link with correct page and poem ID

### Algorithm Summary

```
For each word page:
1. After selecting the balanced N poems, compute their centroid (average embedding)
2. Search all 7,800+ poems for the one closest to the centroid
3. Look up that poem's chronological page (e.g., "05" for page 5)
4. Build anchor link: chronological/05.html#poem-notes-quiet-evening
5. Render header with: Main | Word Cloud | Chronological (pointing to centroid location)
```

### Navigation Result

Each word page now has three header links:
- **Main** → main.html (site navigation hub)
- **Word Cloud** → wordcloud.html (back to cloud)
- **Chronological** → chronological/XX.html#poem-category-id (centroid-based entry point)
