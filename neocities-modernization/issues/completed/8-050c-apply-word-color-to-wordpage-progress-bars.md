...
- Word's semantic color is used for the page header `<h1>`
- Per-poem colors are retained for progress bars

This preserves the intentional color diversity created by 8-050b's balanced
selection algorithm while still giving each word page a clear thematic identity.

### Changes Made

1. **Updated `generate_word_page()` function** (line 698):
   - Added `word_hex_color` parameter
   - Header now uses word's color: `<font color="%s">%s</font>` for the word

2. **Updated call site in `generate_word_html()`** (lines 993-999):
   - Looks up word's semantic color from `word_colors` table
   - Converts color name to hex using `color_config`
   - Passes hex color to `generate_word_page()`

### Visual Result

- Page header "Poems similar to: **silence**" now shows "silence" in blue
- Page header "Poems similar to: **fire**" now shows "fire" in red
- Progress bars still show each poem's individual semantic color (rainbow effect from 8-050b)
