#!/usr/bin/env luajit
-- 122-test-sampler-riscv64.lua
--
-- Choosing what to say next, in the third tongue, on a real emulated RISC-V
...
--
-- usage:
--   luajit 122-test-sampler-riscv64.lua [--dir ROOT] [--seconds N]
...
say("")
say("  choosing the same word, in the third tongue")
say("  " .. string.rep("-", 58))
say("")
...
if host ~= "x86_64" then
  say("  this host is " .. host .. ", and the choices the third tongue is")
...
local riscv_sampler = dofile(DIR .. "/src/120-sampler-riscv64.lua")
local payload = dofile(DIR .. "/src/121-emit-sampler-check-riscv.lua")
...
local text = payload.riscv64({
  ...
  recorded = recorded,
  kernels = riscv,
  sampler_module = riscv_sampler,
  specification = specification,
  float_bits = float_bits,
  dir = DIR,
})

local base = DIR .. "/tmp/shared-memory/payloads/sampler-check-riscv64"
handle = io.open(base .. ".s", "w")
handle:write(text)
handle:close()

if not run_one("clang --target=riscv64-unknown-none -march=rv64imafd -c " .. base .. ".s -o "
               .. base .. ".o") then
  check("the third tongue's sampler assembles", false, "see " .. base .. ".s")
  say("")
  say("  " .. passed .. " of " .. (passed + failed + 1) .. " as expected")
  os.exit(1)
end
check("the third tongue's sampler assembles", true)

-- THE WHOLE REASON 054 EXISTS, checked rather than trusted. If any branch had
-- been written as a branch to a label, the assembler would have left a
-- relocation, extraction would have dropped it, and the offset would have
-- stayed zero -- which is a branch to itself, and a machine that spins
-- forever saying nothing. Checked before the machine is booted rather than
-- diagnosed afterwards from a silence.
...
say("")
say("  what this adds:")
say("    the third architecture can now turn scores into a word. Before")
...
if goodbye then
  goodbye:write("the third tongue chooses: " .. passed .. " of "
