...
| `107b` | Read the card | **not started; scoped twice on 2026-08-08 and now the near path** — cut back when the arithmetic said region-reading is unnecessary while everything fits in memory, then widened again when finding a hard drive turned out to need the same three firmware calls for a better reason. Near: keep the image handle every payload discards at its first instruction, turn off the firmware's five-minute watchdog, and enumerate the attached storage so `206`'s tested machinery meets a real device. Only the region *layout* still waits |
