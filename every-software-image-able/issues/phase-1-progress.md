...
| `105` | The thinking loop, and its limits | **waiting, not in progress** — its assembly twin exists on the first architecture and says the same words, 33 of 33; the readable loop stays the reference. It waits on the other two tongues, and on 2026-08-08 that work turned out to belong to no ticket at all — now `401`'s, named there |
...
| `107a` | The loop, on the metal | **proved, and out of questions** — a machine with nothing underneath it reads, thinks and speaks, word for word what the readable loop speaks, 33 of 33; all three of its open questions were answered on 2026-08-08 and the work each creates belongs to `107b`, `502` and `304` |
| `107b` | Read the card | **in progress — the two small things are done on the first architecture, 2026-09-10** — the machine keeps the firmware's name for itself and spends it at once on proving it is the right name, by asking firmware where this program was placed and requiring the place the code is standing to fall inside it; and the five-minute watchdog is turned off, checked twice over — the status firmware gives back, and a board left sitting for longer than the timer would have run. What remains is the storage: enumerate the attached devices so `206`'s tested machinery meets a real one. Only the region *layout* still waits |
