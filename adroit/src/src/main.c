...
int main(void)
{
    printf("🎮 Starting Adroit RPG Character Generator...\n");
    
    init_random();
    initialize_all_items();
    init_game_state();
    
    // Create initial character to display
    Unit* initial_character = init_unit();
    if (initial_character) {
        update_character(initial_character);
        printf("✅ Initial character created: %s\n", 
               initial_character->name ? initial_character->name : "Unknown");
    }
    ...
    cleanup_game_state();
    cleanup_all_items();
    printf("✅ Adroit shutdown complete\n");
    return 0;
}