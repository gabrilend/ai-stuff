...
| W02e | triggers-on-the-server | W02b | Transpiled triggers run in Eluna. A shim implements JASS natives with Eluna calls (unit creation, timers, region enter events, player resources). Unsupported natives are listed per map, not skipped silently. **This is the rules engine for WC3 maps in the W client** (decided 2026-09-23). The shim and any server module live in their own folder under AGPL v3, because they run inside AzerothCore (AGPL v3) and Eluna (GPL v3); see `docs/licensing-and-boundaries.md` |
