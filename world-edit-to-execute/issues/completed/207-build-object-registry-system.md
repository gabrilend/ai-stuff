...
- [x] ObjectRegistry stores all five object types
- [x] Lookup by creation ID works correctly
- [x] Lookup by name works for named objects
- [x] Filter methods (get_heroes, get_buildings, etc.) work
- [x] Spatial index optional but functional
- [x] Radius queries return correct objects
- [x] Rectangle queries return correct objects
- [x] Integration with Map.load() works
- [x] Unit tests for registry operations
- [x] Unit tests for spatial queries
...
This issue **does not need further splitting** - it has already been split and all sub-issues have been completed.

## Current Status

The issue was previously analyzed and split into 6 sub-issues. All of them are now in `issues/completed/`:
...
| 207f | registry-tests | ✅ Completed |

## Recommendation

This root issue (207) should be moved to `issues/completed/` since all its sub-issues have been completed. The root issue file should have its acceptance criteria checked and an `## Implementation Notes` section added summarizing the completed work before archiving.

Would you like me to:
1. Review the completed sub-issues to verify all acceptance criteria are met?
2. Update the root issue and move it to completed?
...
All 6 sub-issues have been implemented and completed:

### Module Structure
- `src/registry/init.lua` - ObjectRegistry class with all storage and lookup methods
- `src/registry/spatial.lua` - Grid-based SpatialIndex for efficient spatial queries

### Test Coverage
- `src/tests/test_registry.lua` - 71 assertions for core registry operations
- `src/tests/test_spatial.lua` - 75 assertions for spatial index operations
- `src/tests/test_spatial_integration.lua` - 31 assertions for registry+spatial integration
- `src/tests/test_data.lua` - Map.load() integration tests

### Key Features
- 5 object type storage (doodads, units, regions, cameras, sounds)
- By-creation-ID and by-name lookups (O(1) hash table)
- Filter methods: get_heroes(), get_buildings(), get_waygates(), get_units_for_player()
- Spatial index with configurable cell size (default 512 units)
- Radius and rectangle queries
- Automatic spatial indexing on object addition when enabled

### Map.load() Integration
- Map class in `src/data/init.lua` creates registry automatically
- Populates registry from all 5 parser outputs
- Registry accessible via `map.registry`
