...
- [x] Doodad class with visibility/solidity methods
- [x] Unit class with hero/building/item detection
- [x] Region class with bounds checking
- [x] Camera class with eye position calculation
- [x] Sound class with flag accessors
- [x] All classes use metatables
- [x] All classes accept parser output in constructors
...
Looking at this issue, I can see it's well-structured with clear implementation steps for 5 distinct game object types plus a module init. This is a good candidate for splitting because:

1. Each game object type is independent and can be implemented separately
2. The types have different dependencies on parser outputs
3. Splitting allows parallel development and clearer progress tracking

## Recommended Sub-Issues

### 206a-create-gameobjects-module-structure
**Description:** Create the directory structure and init.lua module that exports all game object types. This establishes the foundation that other sub-issues will populate.

**Dependencies:** None (should be done first)

**Scope:**
- Create `src/gameobjects/` directory
- Create `init.lua` with placeholder requires (initially empty/commented)
- Establish the module export pattern

---

### 206b-implement-doodad-class
**Description:** Implement the Doodad class for trees, destructibles, and decorative objects from war3map.doo parser output.

**Dependencies:** 206a (module structure), 201 (doodad parser for input format)

**Scope:**
- `src/gameobjects/doodad.lua`
- Constructor from parsed doodad data
- Methods: `is_visible()`, `is_solid()`, `get_max_life()`
- Unit tests for Doodad class

---

### 206c-implement-unit-class
**Description:** Implement the Unit class for units, buildings, heroes, and items from war3mapUnits.doo parser output.

**Dependencies:** 206a (module structure), 202 (units parser for input format)

**Scope:**
- `src/gameobjects/unit.lua`
- Constructor from parsed unit data
- Methods: `is_hero()`, `is_building()`, `is_item()`, `is_random()`, `is_waygate()`, `get_hero_level()`
- Unit tests for Unit class

---

### 206d-implement-region-class
**Description:** Implement the Region class for trigger areas and waygate destinations from war3map.w3r parser output.

**Dependencies:** 206a (module structure), 203 (regions parser for input format)

**Scope:**
- `src/gameobjects/region.lua`
- Constructor from parsed region data
- Methods: `get_center()`, `get_size()`, `contains_point()`, `has_weather()`, `has_ambient_sound()`
- Unit tests for Region class

---

### 206e-implement-camera-class
**Description:** Implement the Camera class for cinematic and gameplay cameras from war3map.w3c parser output.

**Dependencies:** 206a (module structure), 204 (cameras parser for input format)

**Scope:**
- `src/gameobjects/camera.lua`
- Constructor from parsed camera data
- Methods: `get_eye_position()`, `has_local_rotations()`
- Unit tests for Camera class

---

### 206f-implement-sound-class
**Description:** Implement the Sound class for sound definitions and ambient loops from war3map.w3s parser output.

**Dependencies:** 206a (module structure), 205 (sounds parser for input format)

**Scope:**
- `src/gameobjects/sound.lua`
- Constructor from parsed sound data
- Methods: `is_looping()`, `is_3d()`, `is_music()`, `get_effective_volume()`
- Unit tests for Sound class

---

### 206g-finalize-module-exports-and-docs
**Description:** Update init.lua with all class exports, add documentation comments to public APIs, and ensure all acceptance criteria are met.

**Dependencies:** 206b, 206c, 206d, 206e, 206f (all class implementations)

**Scope:**
- Update `src/gameobjects/init.lua` with real requires
- Add documentation comments to all public methods
- Verify metatable consistency across classes
- Add `__tostring` metamethods for debugging
...
- Sub-issues 206b through 206f can be worked on in parallel after 206a is complete
- Each class sub-issue includes its own unit tests, keeping test coverage tied to implementation
- The finalization sub-issue (206g) ensures everything integrates properly and documentation is complete
- This split allows clear progress tracking: 7 checkboxes instead of one large issue
...
*Generated by Claude Code on 2025-12-27 00:12*
...
All 7 sub-issues have been implemented and completed:

### Module Structure
- `src/gameobjects/init.lua` - Module exports all 5 classes
- `src/gameobjects/doodad.lua` - Doodad class (trees, destructibles)
- `src/gameobjects/unit.lua` - Unit class (units, buildings, heroes, items)
- `src/gameobjects/region.lua` - Region class (trigger areas)
- `src/gameobjects/camera.lua` - Camera class (cinematic cameras)
- `src/gameobjects/sound.lua` - Sound class (audio definitions)

### Test Coverage
- `src/tests/test_gameobjects.lua` - 259 assertions covering all classes
- All metatable patterns consistent across classes
- All classes accept parser output in constructors

### Key Design Decisions
- Items treated as Units with `is_item()` check (matches WC3 data model)
- Runtime state fields (current_hp, etc.) initialized to nil until game starts
- All classes use `__index` metatable for method dispatch
