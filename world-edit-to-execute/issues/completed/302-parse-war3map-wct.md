# Issue 302: Parse war3map.wct (Custom Text Triggers)

**Phase:** 3 - Logic Layer
**Type:** Feature
**Priority:** Medium
**Dependencies:** 102-implement-mpq-archive-parser, 301-parse-war3map-wtg

---

## Current Behavior

Cannot read custom text triggers. JASS code written directly in the trigger
editor (via "Convert to Custom Text") is inaccessible.

---

## Intended Behavior

A parser that extracts custom trigger text from war3map.wct:
- Per-trigger custom JASS code
- Map header comment text
- Trigger comment blocks

---

## Suggested Implementation Steps

1. **Create parser module**
   ```
   src/parsers/
   └── wct.lua          (this issue)
   ```

2. **Implement header parsing**
   ```lua
   -- war3map.wct header structure:
   -- Offset  Type      Description
   -- 0x00    int32     File format version (1 for TFT)
   -- 0x04    int32     Length of map header comment
   -- 0x08    string    Map header comment (custom JASS at top of war3map.j)
   ```

3. **Parse trigger count**
   ```lua
   -- int32:  Number of triggers (should match wtg trigger count)
   ```

4. **Parse trigger custom text entries**
   ```lua
   -- For each trigger (matching wtg order):
   -- int32:  Length of custom text (0 if not custom)
   -- string: Custom JASS text (if length > 0)
   ```

5. **Return structured data**
   ```lua
   return {
       version = 1,
       header_comment = "// Custom map header JASS...",
       triggers = {
           [1] = nil,  -- Not a custom text trigger
           [2] = "function Trig_CustomTrigger_Actions takes nothing returns nothing\n...",
           [3] = nil,  -- Not a custom text trigger
       },
   }
   ```

6. **Link with wtg data**
   ```lua
   -- Provide helper to merge wct data into wtg triggers
   function wct.merge_with_wtg(wct_data, wtg_data)
       for i, trigger in ipairs(wtg_data.triggers) do
           if trigger.is_custom_text then
               trigger.custom_jass = wct_data.triggers[i]
           end
       end
   end
   ```

---

## Technical Notes

### Custom Text Trigger Workflow

In World Editor:
1. User creates GUI trigger
2. User selects "Edit > Convert to Custom Text"
3. Trigger becomes editable as raw JASS
4. GUI representation is lost, only JASS remains

### Header Comment Usage

The header comment appears at the top of the generated war3map.j file.
Common uses:
- Library declarations
- Global function definitions
- Custom type extensions
- Import statements for vJASS

### Relationship to wtg

The wct file is indexed by trigger order from wtg:
- Same number of entries as triggers in wtg
- Entry i corresponds to trigger i in wtg
- Entry is empty (length 0) if trigger is not custom text

### Empty Entries

Non-custom triggers have:
```
int32: 0  (length = 0, no text follows)
```

Custom triggers have:
```
int32: N  (length = N bytes)
char[N]: JASS code
```

---

## Related Documents

- docs/formats/wct-custom-triggers.md (to be created)
- issues/301-parse-war3map-wtg.md (trigger structure)
- issues/303-parse-war3map-j.md (compiled JASS)

---

## Acceptance Criteria

- [x] Can parse war3map.wct from test archives
- [x] Correctly extracts header comment
- [x] Correctly extracts custom trigger text
- [x] Handles empty entries for non-custom triggers
- [x] Provides merge helper for wtg data
- [x] Returns structured Lua table
- [x] Unit tests for parser

---

## Notes

The wct format is simpler than wtg. It essentially stores the raw JASS text
for triggers that were converted to custom text. This data is used:

1. During map save: Custom text is inserted into war3map.j
2. During map load: To restore editable JASS in trigger editor
3. For analysis: To understand what code runs for custom triggers

Without wct parsing, custom text triggers would appear as black boxes.

Reference: [WC3MapSpecification](https://github.com/ChiefOfGxBxL/WC3MapSpecification)

---

## Sub-Issue Analysis

*Generated by Claude Code on 2025-12-19 03:11*

Looking at this issue, I'll analyze whether it benefits from splitting.

## Analysis

This issue covers parsing `war3map.wct`, which has a **simple, linear structure**:

1. Header (version + header comment)
2. Trigger count
3. Array of trigger entries (length + optional text)
4. Optional merge helper

The format is straightforward compared to `war3map.wtg`. The entire file can be parsed in a single sequential pass with no complex branching logic or optional sections.

## Recommendation: **Keep as single issue**

**Reasons not to split:**

1. **Linear parsing** - The file is read top-to-bottom with no optional sections or version-dependent branching
2. **Simple data types** - Only `int32` and null-terminated strings
3. **Small scope** - The implementation is ~50-80 lines of Lua
4. **Tight coupling** - Header, count, and entries are all needed together for a functional parser
5. **Single responsibility** - "Parse wct file" is already atomic

**Comparison to 301 (wtg):**
- WTG has complex nested structures (categories → triggers → functions → parameters)
- WTG has version-dependent parsing paths
- WCT is a flat array with optional string entries

## If you still wanted to split

The only reasonable split would be:

| ID | Name | Description |
|----|------|-------------|
| 302a | wct-core-parser | Header, version, trigger count, basic structure |
| 302b | wct-wtg-integration | Merge helper and integration with wtg data |

But this creates artificial separation - the merge helper is 10 lines and naturally belongs with the parser.

## Suggested approach

Implement as a single issue. The acceptance criteria are well-defined and the implementation is straightforward. If integration with wtg proves complex, that can spawn a follow-up issue at that time.

---

## Implementation Notes

*Completed by Claude Code on 2025-12-27*

### Files Created

| File | Description |
|------|-------------|
| `src/parsers/wct.lua` | WCT parser module (~270 lines) |
| `src/tests/test_wct.lua` | Unit tests with synthetic data (25 tests: 9 synthetic + 16 map tests) |

### Implementation Details

1. **Version Support**
   - Version 0 (Reign of Chaos): No header comment, just triggers
   - Version 1 (The Frozen Throne): Header comment + triggers

2. **String Format**
   - Uses length-prefixed strings (int32 length + raw bytes)
   - NOT null-terminated like some other WC3 formats
   - Empty strings (length 0) indicate GUI triggers (not custom text)

3. **API Functions**
   - `wct.parse(data)` - Parse binary WCT data
   - `wct.extract(archive)` - Extract and parse from MPQ
   - `wct.merge_with_wtg(wct_data, wtg_data)` - Merge custom text into WTG triggers
   - `wct.get_custom_trigger_count(wct_data)` - Count custom triggers
   - `wct.format(wct_data)` - Human-readable summary

4. **Test Coverage**
   - Synthetic data generation (no test maps have wct files - all are protected)
   - Version 0 and 1 parsing
   - Empty/truncated file handling
   - Unicode preservation
   - Large text handling
   - WTG merge functionality

### Observations

All 16 test maps are map-protected and do not contain war3map.wct files. Tests use
synthetic binary data that matches the documented format. Real-world testing will
require unprotected maps with GUI triggers.

### Related Changes

- Created `tmp/check_wct.lua` for map inspection (can be removed after verification)
