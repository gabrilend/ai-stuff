...
- [x] Can dump info from any test .w3x file
- [x] Text output is human-readable
- [x] JSON output is valid JSON
- [x] Component filtering works (-c flag)
- [x] Interactive mode works (-I flag)
- [x] Output to file works (-o flag)
- [x] Helpful error messages for invalid inputs
- [x] Help text explains all options
...
Created `src/cli/mapdump.lua` - a command-line tool for inspecting WC3 map files:

```bash
# Basic usage - dump all info
lua5.4 src/cli/mapdump.lua map.w3x

# JSON output
lua5.4 src/cli/mapdump.lua map.w3x -f json

# Specific component
lua5.4 src/cli/mapdump.lua map.w3x -c terrain
lua5.4 src/cli/mapdump.lua map.w3x -c strings
lua5.4 src/cli/mapdump.lua map.w3x -c info

# Output to file
lua5.4 src/cli/mapdump.lua map.w3x -o output.txt

# Interactive mode
lua5.4 src/cli/mapdump.lua -I
```

### Features

- **Components**: info, strings, terrain, files, all
- **Formats**: text (human-readable), JSON (machine-readable)
- **Interactive mode**: Menu-based selection of map, component, format
- **File output**: -o flag for writing to file
- **JSON encoder**: Built-in simple JSON encoder (no dependencies)
- **Error handling**: Clear error messages for missing files, invalid options
...
Name: Dark Ages of Warcraft 2.1
Author: Luka_Blight and Clan BLV
Dimensions: 256x256 tiles
Tileset: cityscape (Y)

=== Players (12) ===
[0] Red - Thrall's Horde (human, selectable)
    Start: (-9216, 5632)
...
```
...
- `src/cli/mapdump.lua` - Main CLI tool (~630 lines)
