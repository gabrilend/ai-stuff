...
- [x] `stream_queue()` function exists
- [x] Displays outputs in order (by queue index)
- [x] Shows formatted header with issue name
- [x] Respects `STREAM_DELAY` between outputs
- [x] Terminates properly when done file exists and all items processed
- [x] Polls efficiently (0.2s interval when waiting)
...
Added `stream_queue()` function at lines 123-176 in issue-splitter.sh:
- Uses local `stream_idx` variable (avoids subshell scope issues)
- Polls for `.ready` files in order (0, 1, 2...)
- Displays formatted header with issue basename and [success/failed] status
- Uses `STREAM_DELAY` variable (default 5s) between outputs
- Termination via idle counter when done file exists (5s timeout)
- Poll interval 0.2s when waiting for next item

### Design Decisions

- Local stream_idx instead of global STREAM_INDEX (subshell-safe)
- Idle counter timeout (25 polls = 5s) ensures termination even if
  producer fails to signal correctly
- Shows status in header for quick visual feedback on failures

### Testing

Created `src/tests/test_002c_streamer_process.sh` which verifies:
- Outputs displayed in correct order
- Status shown in headers ([success]/[failed])
- Terminates promptly when done
