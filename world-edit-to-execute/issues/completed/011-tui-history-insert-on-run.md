...
- [x] TUI mode can exit without executing, returning user to terminal
- [x] Built command is displayed clearly for user to copy/recall
- [x] Optional: clipboard copy via config file (COPY_TO_CLIPBOARD=true)
- [x] User can copy command from output
- [x] Works correctly with all option combinations
- [x] Special characters handled by TUI's existing command preview
...
- Added `PRINT_COMMAND=false` configuration variable
- Added `COPY_TO_CLIPBOARD=false` user-configurable variable
- Added config file loading from `~/.config/issue-splitter/config`
- Added `-P|--print-command` flag to parse_args
- Added print-command logic in interactive_mode_tui that:
  - Gets command from `menu_get_value "cmd_preview"`
  - Prints it in a nice box
  - Optionally copies to clipboard (xclip/xsel/pbcopy)
  - Exits cleanly

### Usage

```bash
# Run TUI, print command instead of executing
./issue-splitter.sh -I -P

# Enable clipboard copy (create config file)
...
*Generated by Claude Code on 2025-12-27 00:36*
