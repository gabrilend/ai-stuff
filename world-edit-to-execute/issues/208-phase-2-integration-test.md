...
*Generated by Claude Code on 2025-12-16 05:38*

Looking at this issue, I'll analyze whether it benefits from splitting into sub-issues.

## Analysis

This is an **integration test issue** that ties together all of Phase 2. The suggested implementation steps are already well-organized, but they represent distinct testing concerns that could be developed and validated independently.

**Reasons to split:**
1. Each test category (parsers, objects, registry, cross-refs) can be developed independently
2. The demo script is a separate deliverable from the test suite
3. Splitting allows parallel work and clearer progress tracking
4. Individual test failures will be easier to diagnose

**Reasons to keep as-is:**
1. Integration tests by nature should test things *together*
2. The components are tightly coupled - you need parsers before objects, objects before registry
3. The issue is already a "capstone" for the phase

## Recommendation: Split into 4 sub-issues

The issue has clear sequential dependencies but distinct deliverables. Splitting helps track progress through what is actually a substantial amount of test code.
...
|----|------|-------------|--------------|
| **208a** | `parser-integration-tests` | Test all Phase 2 parsers loading from MPQ archives together. Validates doo, unitsdoo, w3r, w3c, w3s parsers work with real map files. | 201-205 (parsers) |
| **208b** | `gameobject-creation-tests` | Test game object instantiation from parsed data. Validates Doodad, Unit, Region, Camera, Sound objects create correctly with proper methods. | 206, 208a |
| **208c** | `registry-integration-tests` | Test ObjectRegistry population and queries. Validates adding objects, lookups by creation ID, spatial indexing, and cross-reference resolution (waygates↔regions, regions↔sounds). | 207, 208b |
| **208d** | `phase2-demo-script` | Create visual demo script for Phase 2 capabilities. Produces formatted output showing map statistics, object breakdowns, and spatial query results. Includes bash runner script. | 208a, 208b, 208c |
...
201-205 (parsers)
  ...
  207 (registry system)
    │
    ▼
  208c (registry integration + cross-refs)
    │
    ▼
  208d (demo script)
```
...
- **208a** is the foundation - if parsers don't integrate, nothing else works
- **208b** can start once 208a passes basic tests, even if edge cases remain
- **208c** is where the real "integration" happens - cross-reference validation
- **208d** is intentionally last as it demonstrates the working whole

The root issue (208) should remain as the tracking issue, marked complete only when all sub-issues pass.
...
| ID | File | Description |
|----|------|-------------|
| 208a | `208a-parser-integration-tests.md` | Test all Phase 2 parsers loading from MPQ archives |
