# Conversation Summary: fe005677-1798-43b9-877e-d6f2ddc6cc60
...
/mnt/mtwo/programming/ai-stuff/world-edit-to-execute/issues/302-parse-war3map-wct.md
...
5. **Return structured data**
   ...
   return {
       ...
       triggers = {
           ...
           [2] = "function Trig_CustomTrigger_Actions takes nothing returns
nothing\n...",
...
This issue covers parsing `war3map.wct`, which has a **simple, linear
structure**:
...
The format is straightforward compared to `war3map.wtg`. The entire file can be
parsed in a single sequential pass with no complex branching logic or optional
sections.
...
1. **Linear parsing** - The file is read top-to-bottom with no optional sections
or version-dependent branching
2. **Simple data types** - Only `int32` and null-terminated strings
3. **Small scope** - The implementation is ~50-80 lines of Lua
4. **Tight coupling** - Header, count, and entries are all needed together for a
functional parser
...
But this creates artificial separation - the merge helper is 10 lines and
naturally belongs with the parser.

## Suggested approach

Implement as a single issue. The acceptance criteria are well-defined and the
implementation is straightforward. If integration with wtg proves complex, that
can spawn a follow-up issue at that time.
