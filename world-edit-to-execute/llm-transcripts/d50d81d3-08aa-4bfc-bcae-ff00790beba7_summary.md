# Conversation Summary: d50d81d3-08aa-4bfc-bcae-ff00790beba7
...
/mnt/mtwo/programming/ai-stuff/world-edit-to-execute/issues/108-phase-1-integration-test.md
       ...
       for _, filename in ipairs(list_files(maps_dir, "%.w3[xm]$")) do
           ...
           if not archive then
               ...
               for _, required in ipairs({"war3map.w3i", "war3map.w3e"}) do
                   ...
                   if not data then
                       table.insert(result.errors, "Extract failed: " ..
required)
                   end
               end
...
3. **Create demo script**
   ...
   for i, filename in ipairs(maps) do
       ...
       for _, player in ipairs(map.players) do
           print(string.format("│   [%d] %s (%s)", player.id, player.name,
player.race))
       end
...
This issue is a good candidate for splitting. It contains four distinct
deliverables that can be implemented independently:
...
This split allows parallel work on 108a/108b if needed (they use same modules
but produce different outputs), and clearly separates the "make it work" phase
from the "document and finalize" phase.
