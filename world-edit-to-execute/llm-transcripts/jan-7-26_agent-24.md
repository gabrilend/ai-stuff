# Conversation Summary: 4ddee3e7-b878-4877-b3c5-297a22bfc59f
...
7. **Implement player events**
   ...
   function runtime.TriggerRegisterPlayerChatEvent(trigger, player, message,
exact)
       return events.registry:register(
           EVENT.PLAYER_CHAT,
           trigger,
...
This issue is well-suited for splitting. It covers multiple distinct subsystems
(core registry, timer events, region events, unit events, player events) that
can be implemented and tested independently. Each event category has different
context structures and filtering logic.
...
2. **Parallel development** - After 308a, sub-issues b/c/d/e have no mutual
dependencies
3. **Natural boundaries** - Each maps to a distinct JASS API surface area
4. **Incremental integration** - Can verify trigger→event→action flow with just
timers before adding combat events
...
- .//issues/308a-implement-event-registry-core.md
- .//issues/308b-implement-timer-events.md
- .//issues/308c-implement-region-events.md
- .//issues/308d-implement-unit-events.md
- .//issues/308e-implement-player-events.md

--------------------------------------------------------------------------------

### Assistant Response 1

All 5 sub-issue files now exist:

**Created issue documentation files:**

1. **308a-implement-event-registry-core.md** - Already existed with valid
content defining the core EventRegistry class, EVENT constants, and
register/unregister/fire methods

2. **308b-implement-timer-events.md** - Timer subsystem with:
   - TriggerRegisterTimerEvent function
   - One-shot and periodic timer support
   - update_timers(dt) game loop hook
   - Timer control functions (Pause/Resume/Destroy)
   - Timer accessors (GetElapsed/Remaining/Timeout)

3. **308c-implement-region-events.md** - Region entry/exit events with:
   - TriggerRegisterEnterRegion/LeaveRegion functions
   - unit_entered_region/unit_left_region fire hooks
   - GetEnteringUnit/GetLeavingUnit/GetTriggerRegion accessors
   - Compound filtering (region + optional unit filter)

4. **308d-implement-unit-events.md** - Combat and unit action events with:
   - TriggerRegisterUnitEvent for specific units
   - TriggerRegisterAnyUnitEventBJ for any unit
   - TriggerRegisterPlayerUnitEvent for player's units
   - Fire hooks for death, damage, attack, spawn, orders, spells
   - Context accessors (GetDyingUnit, GetKillingUnit, GetEventDamage, etc.)

5. **308e-implement-player-events.md** - Player interaction events with:
   - TriggerRegisterPlayerChatEvent with exact/substring matching
   - TriggerRegisterPlayerAllianceChange
   - player_chat/player_left/player_alliance_changed fire hooks
   - GetTriggerPlayer, GetEventPlayerChatString accessors
   - SubString helper for WC3 0-indexed compatibility
