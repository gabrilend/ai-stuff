...
- [x] Profession registry can define profession types
- [x] Profession component attaches to entities
- [x] Entities can learn professions
- [x] Entities can unlearn professions
- [x] Primary profession limit enforced
- [x] Skill levels track correctly
- [x] Skill-up mechanics work (difficulty colors)
- [x] Multiple skill formulas supported (WoW, WC3, custom)
- [x] Cooldowns track per-profession
- [x] Events fire for learn/unlearn/skillup
...
- `src/runtime/systems/professions.lua` - Core profession system
- `src/tests/test_professions.lua` - Unit tests (57 tests, all passing)

### Features Implemented

1. **Profession Type Registry** (lines 126-256)
   - `register_type(definition)` - Register profession type with metadata
   - `get_type(id)` - Get profession definition
   - `get_all_types()` - Get all registered professions
   - `get_types_by_category(type)` - Filter by primary/secondary
   - Default professions registered on init (mining, blacksmithing, cooking, etc.)

2. **Per-Profession Cooldowns** (lines 949-1101)
   - `set_cooldown(entity, profession, name, duration)` - Start a cooldown
   - `get_cooldown_remaining(entity, profession, name)` - Check time left
   - `is_cooldown_ready(entity, profession, name)` - Check if ready
   - `clear_cooldown(entity, profession, name)` - Clear a cooldown
   - `get_all_cooldowns(entity, profession)` - Get all active cooldowns
   - `cleanup_expired_cooldowns(entity, profession)` - Cleanup expired

3. **New Events**
   - `COOLDOWN_STARTED` - Fired when cooldown begins
   - `COOLDOWN_READY` - Fired when cooldown expires or is cleared

### Test Summary

| Category | Tests |
|----------|-------|
| Configuration Modes | 5 |
| Learn/Forget Professions | 7 |
| Skill Levels | 6 |
| Skillup Chance | 5 |
| try_skillup | 3 |
| Recipes | 3 |
