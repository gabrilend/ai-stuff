...
local band, bor, bnot, bxor = compat.band, compat.bor, compat.bnot, compat.bxor
...
local function init_crypt_table()
    ...
    for index1 = 0, 255 do
        ...
        for _ = 0, 4 do
            ...
            local temp1 = lshift(band(seed, 0xFFFF), 16)

            seed = (seed * 125 + 3) % 0x2AAAAB
            local temp2 = band(seed, 0xFFFF)

            crypt_table[index2] = bor(temp1, temp2)
            index2 = index2 + 256
        end
    end
...
-- {{{ to_unsigned
-- Converts a signed 32-bit int to unsigned (for LuaJIT compatibility)
...
function hash.mpq_hash(str, hash_type)
    ...
    for i = 1, #str do
        ...
        seed1 = bxor(crypt_table[table_index], band(seed1 + seed2, 0xFFFFFFFF))
        seed2 = band(ch + seed1 + seed2 + lshift(seed2, 5) + 3, 0xFFFFFFFF)
    end

    -- Convert to unsigned for consistent comparison
    return to_unsigned(band(seed1, 0xFFFFFFFF))
end
-- }}}
-- }}}
...
function hash.decrypt_block(data, key)
    ...
    for i = 1, #data, 4 do
        ...
        seed2 = band(seed2 + crypt_table[0x400 + band(seed1, 0xFF)], 0xFFFFFFFF)

        -- Read encrypted value
        local encrypted = compat.unpack_uint32(data, i)

        -- Decrypt
        local decrypted = band(bxor(encrypted, band(seed1 + seed2, 0xFFFFFFFF)), 0xFFFFFFFF)

        -- Update seeds for next iteration
        seed1 = band(bor(lshift(bnot(seed1), 21) + 0x11111111, rshift(seed1, 11)), 0xFFFFFFFF)
        seed2 = band(decrypted + seed2 + lshift(seed2, 5) + 3, 0xFFFFFFFF)

        -- Pack decrypted value back to bytes
        local b1 = band(decrypted, 0xFF)
        local b2 = band(rshift(decrypted, 8), 0xFF)
        local b3 = band(rshift(decrypted, 16), 0xFF)
        local b4 = band(rshift(decrypted, 24), 0xFF)
        result[#result + 1] = string.char(b1, b2, b3, b4)
    end
