...
function Room:update(dt)
    ...
    if player_units > 0 and enemy_units == 0 and self.owner ~= 1 then
    ...
    elseif enemy_units > 0 and player_units == 0 and self.owner ~= 2 then
        self.capture_progress = self.capture_progress + dt
        if self.capture_progress >= self.capture_time then
            self.owner = 2
            self.capture_progress = 0
    ...
    elseif player_units > 0 and enemy_units > 0 then
        -- Mixed units - pause capture progress, don't reset
    elseif player_units == 0 and enemy_units == 0 then
