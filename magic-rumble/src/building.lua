...
function Building:activate_effect()
    ...
    elseif self.type.name == "Shop" then
        -- Shop effect - generates bonus gold for the player
        if self.owner == 1 then
            player_add_gold(2)  -- Shop generates 2 bonus gold
        elseif self.owner == 2 then
            enemy_add_gold(2)  -- Enemy shop generates gold too
        end
    elseif self.type.name == "Inn" then
        -- Inn effect - heals nearby units in the same room
        if self.room then
            for _, unit in ipairs(self.room.units) do
                if unit.owner == self.owner and unit.health < unit.max_health then
                    unit.health = math.min(unit.max_health, unit.health + unit.max_health * 0.3)
                end
            end
        end
    end
