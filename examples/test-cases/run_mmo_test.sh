...
LOG_FILE="examples/test-cases/mmo_test_results.log"
rm -f "$LOG_FILE"
...
test_offline_mode() {
    ...
    echo "3" | timeout 3s ./files/target/release/mmo-demo > /tmp/offline_test.out 2>&1
...
test_azerothcore_mode() {
    ...
    echo "1" | timeout 3s ./files/target/release/mmo-demo > /tmp/server_test.out 2>&1
...
test_p2p_mode() {
    ...
    echo "2" | timeout 3s ./files/target/release/mmo-demo > /tmp/p2p_test.out 2>&1
...
test_world_generation() {
    ...
    echo "3" | timeout 2s ./files/target/release/mmo-demo > /tmp/world_test.out 2>&1
...
test_ascii_rendering() {
    ...
    echo "3" | timeout 2s ./files/target/release/mmo-demo > /tmp/render_test.out 2>&1
