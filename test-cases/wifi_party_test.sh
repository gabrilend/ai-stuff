...
LOG_FILE="test-cases/wifi_party_results.log"
rm -f "$LOG_FILE"
...
test_party_startup() {
    ...
    echo "4" | timeout 3s ./target/release/mmo-demo > /tmp/party_host.out 2>&1 &
    sleep 2
...
test_device_joining() {
    ...
    echo "5" | timeout 3s ./target/release/mmo-demo > /tmp/party_join.out 2>&1 &
    sleep 2
...
simulate_party_scenario() {
    ...
    echo "4" | timeout 2s ./target/release/mmo-demo > /tmp/laptop_host.out 2>&1 &
    sleep 1
    
    # Simulate multiple Anbernic devices joining
    for i in {1..3}; do
        log "🎮 Anbernic $i: Joining party..."
        echo "5" | timeout 2s ./target/release/mmo-demo > /tmp/anbernic_$i.out 2>&1 &
        sleep 0.5
    done
