...
env RUST_LOG=info ./target/release/daemon > "$LOG_DIR/music-daemon.log" 2>&1 &
DAEMON_PID=$!
sleep 2
...
./target/release/music-demo < "$LOG_DIR/piano_session.txt" > "$LOG_DIR/piano_player.log" 2>&1
...
./target/release/music-demo < "$LOG_DIR/drum_session.txt" > "$LOG_DIR/drum_player.log" 2>&1
