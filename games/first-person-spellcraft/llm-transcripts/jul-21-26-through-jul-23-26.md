...
- You mostly *don't* play a fixed hero. You outfit **NPCs (the vision calls them
   "New Character People" / NCP)** by editing **templates, never instances** —
  "here, have a health potion, there's more at the stockpile." They venture out,
      fight zombies/monsters, and bring back gold, gems, and *trial logs*, which
        they spend at **markets you've placed** (how many lumber shops, how many
                      workers — fewer = slower throughput but room to spread).
    - Monsters build **lairs**: ~3 puzzles + exactly 4 combats each, **generated
    fresh per party** by a strong local AI, which the NPCs solve with a *weaker*
      model. The DM-AI **remembers how capable each character is** and re-scales
   what "a level" means. Characters visiting the library absorb fairy-tales that
  teach real mechanics (quaternions, Newton's laws as "magical histories") which
        make later puzzles easier. Failed puzzles spring **traps** — sometimes
                                             disarming the trap *is* the puzzle.
        - LLM **companion speech patterns** grow between runs to fight character
  burnout; you can save a grown pattern as a new (summarized) starting template.
       - Spells = **Dominions spell list**, many spells per level/path, multiple
                        casting methods each, anything aimable is aimed by hand.
   - Politics follow **Majesty**: neighboring provinces give resources if you're
    kind, become training challenges if you're cruel, revert to monsters if left
       alone — and if you're cruel to *too many*, **they unionize and come end
                                                                          you**.
   - Delivery dreams: run it on an **Anbernic** handheld, "give one copy to each
         European," and a wild idea to encode the game to **cassette audio** fed
                                     through a Game Boy interface, PICO-8 style.
...
story. Named-narrative functions, vimfolded, continued via `include` only when a
  ...
  A couple of things I'd want you to decide before any docs get written, because
    ...
    1. The build note wants **one giant contiguous function/file**, but the game
       ...
       companion voice) plus **threads** and a real-time dual-mouse render loop.
       Those pull in opposite directions — a single linear main() vs. a thread
         pool of coroutines. How do you picture the strong-DM-AI and the per-NPC
      weak-AI actually running relative to that one big function — as blocking
     ...
     two-mouse aiming body in an empty Doom room, or the NPC-template-and-market
       economy loop with no combat yet? They barely share code, so whichever you
                                                      name first shapes phase 1.
...
> plus threads
> and a real-time dual-mouse render loop. Those pull in opposite directions —
> a single
...
> 2. What's the thinnest playable core you'd want to prove first — the
> two-mouse aiming
> body in an empty Doom room, or the NPC-template-and-market economy loop with
> no combat
...
>   Found it — it's a real C system with a `soramech-pool` binary. Let me read
>   its docs and source to understand the ring-buffer/trigger pattern precisely.

>      This is exactly the pattern. Let me read the slot-store and dispatch info
>  files — those hold the ring-buffer/per-consumer-index detail you described.
...
## How the ring-buffer/trigger model answers Q1
 ...
 - **Every LLM call is a box.** A box is just a function with named input ports.
      It does *not* fire when someone calls it — it fires when **all its input
     ports have a value waiting**. That's the "trigger style" you described: the
  strong puzzle-maker box sits dark until its inputs (party-capability estimate,
     room seed, difficulty target) have all arrived, and only then does the pool
         pick it up. The weak per-NPC solver is a separate box firing on its own
     trigger. They run on different worker threads *by default* because the pool
   just grabs whatever box is ready — nobody orchestrates the threads by hand.

           - **Values move through per-port ring buffers, never through assigned
  variables.** Each input port owns one slot (a small ring of cells). A producer
  `push`es a value in; the consumer `pop`s it out FIFO. So there is no `x = f()`
    anywhere — a wire is a ring buffer, and "setting a variable" is "pushing a
                                                                          cell."

- **The index you described is the atomic-counter slot.** SoraMech has a special
  slot whose whole job is `read-and-increment`: fetch the counter, add one, hand
      back the pre-increment number — atomically, no lock. That's exactly "the
  ring buffer itself increments the index so one address goes to each consumer."
  When one producer fans out to many identical solver-workers, the counter deals
                 each worker a distinct index so no two grab the same work item.
...
## The FPS-first slice — and the one hard problem it forces
 ...
 Thinnest core: **two mice driving a two-handed body in one empty square room.**
   ...
   But here's the sharp technical problem, and I want your read on it before I'd
       ...
       **SoraMech is built to reach *quiescence and exit*. A game does the exact
   opposite — it must never quiesce; it re-fires 60 times a second, forever.**
     The runtime already has the hook for this (a box that *re-arms itself after
 firing* keeps the runner alive until Ctrl-C — the "timer box"). So the game's
       heartbeat is a **frame-clock box** that, every ~16 ms, pushes a tick that
    triggers: read-both-mice → solve-body-pose → render. It re-arms, and the
                                                                     loop lives.
...
ring buffer between the mouse thread and the pose-solver could hold *many* delta
  ...
  - **Peek-latest** — a 1-cell slot, pose-solver reads only the newest sample.
   Simple, but you *lose motion* — fast flicks get dropped, aiming feels laggy
                                                                      and wrong.
  - **Drain-and-sum** — a real ring, pose-solver pops *every* delta since last
  frame and adds them up. Correct for aiming (you integrate all the motion), but
                 the pose box is now a multi-spawn drainer, not a fire-once box.
...
per-frame accumulator. That's the crux of fitting an FPS into a dataflow engine.
      ...
      LLM/economy machinery (one runner, one graph, the render loop just another
       re-arming box) — or is the real-time renderer a *separate* fast process
    that the dataflow map feeds, so the 60 Hz loop never has to touch quiescence
                                                                         at all?
...
> and adds
> them up. Correct for aiming (you integrate all the motion), but the pose box
> is now a
...
>   Two clear things here: **(1)** an explicit task — update the SoraMech docs
>     to reflect that long-running circular maps are the *primary* purpose, then
>    commit as "clarifying the system purpose". And **(2)** confirmed FPS design
>                     decisions to hold onto. Let me do the SoraMech work first.
>
>      First, let me check the repo state so my commit stays clean, and read the
>                                                             table of contents.

>    The soramech repo has a large amount of unrelated in-progress work (phase-4
>     scaffolding, new issue files, transcript exports) — none of it mine, and
> none of it staged. Per your separation-of-concerns rule I'll leave all of that
...
>
>    Now the edits. Reframing quiescence from "the end" to "one of two fates" in
>                                                                  the overview:
...
>          Let me check whether the HTML doc mirror contains a now-contradictory
>                            quiescence framing worth fixing in the same commit:
...
>    **iterator box** (or a re-arming heartbeat like the timer box). Let me read
>       the exact mechanism so I document what's real, not an idealized version.

>    That's the accurate picture, and it means my first draft was wrong in a way
>     worth fixing: a plain back-edge is **rejected** at load time. A legal loop
> must pass through an **iterator box** (which re-fires while its queue drains),
>  and the endless wall-clock heartbeat — the **timer box** — is still issue
>    251, a *design draft*, not yet built. Let me correct both edits so the docs
>                             describe the real mechanism, not an idealized one.
...
Your answers land on a clean two-tier rule for how values travel — this is the
   ...
   - **Between compute boxes** (mice → pose-solver, and the LLM pipelines) →
  **ring-buffer queues, every value matters.** Mouse input is drain-and-sum with
      a **ring of 3–5 cells**: each frame the pose box pops *all* deltas since
    last frame and sums them, so no flick is lost. The pose box is a multi-spawn
                                                   drainer, not a fire-once box.
         - **Into the renderer** (pose/state → render list) → **single-cell,
        latest-wins, overwrite is fine.** The renderer is *one raylib thread*, a
    constant consumer that sweeps "everything to draw with this rendering type,"
   then the next type. Boxes just stamp each renderable's current state into its
   slot; no queue, no zeroing, no history. One position holds one thing. The GPU
                  builds the scene from those data-derived positions each frame.
...
> One small thing to keep an eye on (not a blocker, and your "overwriting is
> fine" stance
> already accepts it): if a box writes a multi-field renderable (x, y, z,
> facing) while the
...
world spatially, then only touch the segments the current view can reach. You're
  ...
  condition that makes it airtight, because there's a version of this that still
     ...
     - **The version that defeats tearing:** an object moves by (a) building its
   whole new record — x, y, z, facing, all of it — off to the side, then (b)
      **splicing its index into the sorted list as the single, last write**. The
    list *is* the publication point. The render thread walks the index and reads
  complete records, so it either sees the object at its old sorted position (old
        whole state) or its new one (new whole state) — never a half. The sort
  insertion is the atomic hand-off, the same way a pointer swap double-buffers a
                                                                          frame.

  - **The version that still tears:** the render thread finds the object via the
   index, then reads `x, y, z, facing` *live* off a record that a compute box is
    concurrently overwriting field by field. Culling narrowed *which* objects it
      reads, but each read is still exposed. Fewer objects tearing, but tearing.
 ...
 index is in the list.** Is that the shape you're picturing — the index splice
  ...
  1. The cull runs "whenever the screen is moved." In this game the camera moves
         ...
         per-frame — but what about an **object that moves into view while the
        camera is still** (an NPC walking on-screen while you hold aim)? Does an
       object's own move also re-trigger the visibility pass, or does the render
               just walk the current index every frame regardless of what moved?

     2. The x/y sort gives an axis-aligned box; the view is an angular cone. For
       objects **right at the cone's edge**, are you happy to over-draw a little
     (draw the bounding-box surplus and let the angle test skip them per-frame),
                        or do you want the angle band folded into the index too?
...
> new record
> — x, y, z, facing, all of it — off to the side, then (b) splicing its
> index into the
> sorted list as the single, last write. The list is the publication point. The
> render
> thread walks the index and reads complete records, so it either sees the
> object at its
> old sorted position (old whole state) or its new one (new whole state) —
> never a half.
...
>          That's a lot of good signal. Let me survey what already exists in the
>    spellcraft project before I create anything (so I don't duplicate structure
>          and I respect the indexed-filename convention), then I'll answer your
>                                              repeat-back and the git question.
...
>       and our SoraMech-dataflow conversation is the architecture decision that
>       belongs *in* issue 101. Let me read the ones that govern the engine core
>   before I write a line, so the implementation matches the existing blueprints
...
>                fixed-timestep loop. Our last several turns evolved that into a
>  **SoraMech-style dataflow substrate** on **raylib**, with a data-driven scene
>   and sorted-index culling. Issue 101 is literally "the architecture decision,
>    the taproot" — so that's exactly where this evolution has to be recorded,
>                                                     not forked into new files.
>
...
>  `src/soramech/` story starting at `000`. The global `.file-index-counter` (4)
>        is already out of step with that practice; I'll follow the demonstrated
>                per-module pattern and leave that counter for you to reconcile.
>
...
>         FIFO-full backlog signal, drain-and-sum, and the fan-out index) before
>                                                       building anything on it:

>     Real bug, caught by the test: my `is_full` reports a `latest` slot as full
>     once it holds a value, contradicting the documented "latest is never full"
>     contract. The fix is to make the contract true in code — a `latest` slot
>     always has room because overwriting *is* the operation, so there's never a
>                                                 reason to look before pushing.

> All 16 pass, including the drain-and-sum and fan-out-index behaviors. The atom
>                                                                      is solid.
>
>     Let me place the new notes doc into the table of contents (house rule: new
>                docs get indexed there). Reading it first to slot it correctly:

>    The datapath docs are actually all present now but still marked *(planned)*
>       here — that's stale, but it's a separate cleanup I'll leave for you to
>     confirm rather than sweep in. Adding the new notes doc to the tree and the
>                                                                 document list:
...
## Your splice design, repeated back — did it make sense?
         ...
         - **Two parallel arrays sharing one physical index:** a `value[]` and a
              count-carrying `index[]`. `value[i]` and `index[i]` ride together.
 - **`index[loc]` is a bucket count.** `index[57] = 2` means the sorted position
   57 is a *rounded bucket* holding two real values — the second one living at
  56, i.e. "somewhere between 56 and 57, rounded." Collapsing neighbors into one
     bucket trades exact intra-bucket order (which rough front-to-back rendering
                           doesn't need) for less storage and cheaper insertion.
  - **Splice = bump the count at the location + bump the published total + write
                      the record** — instead of an O(n) memmove to open a gap.
...
counts mark which consecutive physical slots are "the same rounded position," so
    ...
    **One inversion to flag** (this is the tearing-relevant bit): you stated the
  *principle* exactly right — "splice its index in as the single, last write."
    But the *ordering* you then described is "bump the count and total, **then**
write the data." Those fight. If the published total goes up *before* the record
   is written, a consumer reading `[0, total)` can reach the new slot while it's
         still empty — publish-before-populate, a torn read. To honor your own
  principle, the record write has to come **first**, and the count/total bump is
      the single **last** write that publishes it. Same rule as the record-level
     ownership handoff, just applied to the collection. Did you mean count-last?

      **And one spot I genuinely can't resolve yet** — the payoff of the whole
scheme: when you write the new value into `value[]` at its bucket, how does that
avoid shifting everything *after* it? Bumping `index[57]` to 2 says "57 is now a
  2-wide bucket," but the physical byte still has to land somewhere in the dense
array, and if slot 58 is occupied you're back to a tail shift. Is the trick that
  the new value sits at the *end of its run* and you accept the bucket got wider
   (so no tail moves, order just gets rounder)? Or is there still a shift you're
paying? That's the hinge the O(1)-insert claim rests on, and I want to make sure
                                                     I see it before I build it.
 ...
 **Gamestate vs. renderables:** got it — two structures. *Gamestate* is truth,
    rewritten every time anything moves. *Renderables* is what the render thread
        walks, and it's refreshed on **camera move**, when visible objects write
          themselves in. Clean separation (it's pattern 7 in the notes). The one
      consequence, now a known/accepted trait: with a perfectly still camera, an
     on-screen object that moves won't re-appear in `renderables` until the view
 nudges — invisible during twitch aiming, but worth remembering for the "stand
                               still and watch your NPCs" Majesty moments later.

 **Edge-records:** got it — each object contributes left/right (and top/bottom
     if vertical matters) edge records that reference each other and **move as a
   unit**. That's an interval/span in the sorted index, so a partially-on-screen
    object is caught by whichever edge is in view. Over-draw at the cone edge is
                    handled by the span, not by folding the angle into the sort.

## The git question — tags, symlinks, or a branch?

                                              Straight answers on the mechanics:

- **Git can't tag a file.** A tag is a named pointer to a *commit* (a whole tree
   snapshot), not to a path. There's `git notes` (attaches text to a commit) and
   `.gitattributes` (per-path flags), but neither is "tag this file as part of a
                                           set." So tagging isn't the tool here.
    - **A committed symlink stores only its target *path*, not the bytes.** If I
     symlink a file inside SoraMech to `../../spellcraft/docs/soramech-notes.md`
        and commit it, SoraMech's history contains the *string* of that path —
       anyone cloning SoraMech alone gets a dangling link, and the notes' actual
    content and history never enter SoraMech. Works locally, fragile as a shared
                                                                       artifact.
 - **Your "a branch that's JUST this documentation" instinct is the right tool**
     — an **orphan branch**: its own root, sharing zero history with `master`,
        holding only a real copy of the notes. Update and commit there when ours
      changes; `git checkout master` and the code is untouched. It's exactly the
                                                             `gh-pages` pattern.

       **Recommendation:** an orphan branch in SoraMech (say `spellcraft-notes`)
holding a real copy. Because SoraMech's working tree is full of your uncommitted
   work right now, I'd build and maintain it through a **dedicated worktree** (a
     throwaway second checkout in a temp dir) so I never switch branches in your
          dirty main tree. Say the word and the branch name, and I'll set it up.
...
## What I built, and the one thing I need ratified before pouring more
  ...
  - `src/soramech/000-ring-buffer-slot.lua` + `.info.md` — the atom every wire
     is made of: the three flavors (FIFO `queue` for drain-and-sum, `latest` for
                the renderer blackboard, `counter` for fan-out indexing) and the
     build-then-publish ownership contract. **16 smoke-checks pass** — and the
   test earned its keep: it caught a real bug (a `latest` slot wrongly reporting
                                                         "full"), which I fixed.
                                                  - TOC updated for the new doc.

 **What I deliberately did *not* touch:** issues **101** and **104**. And here's
                why I stopped — surveying the plan surfaced a real divergence:
...
decision into 101 and pour the next layers (the box + trigger-on-ready dispatch,
  ...
  3. **Phase-1 threading** — my recommendation: **single thread, cooperative**
   ...
   boundary built pool-ready so true parallelism + a separate render thread drop
     in later when the LLM boxes (phase 6) actually demand it. That's SoraMech's
   own v1-synchronous-→-pool path. Confirm, or do you want the separate render
                                                            thread from day one?
...
> you're back to
> a tail shift. Is the trick that the new value sits at the end of its run and
> you accept
> the bucket got wider (so no tail moves, order just gets rounder)? Or is there
> still a
> shift you're paying? That's the hinge the O(1)-insert claim rests on, and I
> want to make
...
> - A committed symlink stores only its target path, not the bytes. If I symlink
> a file
> inside SoraMech to ../../spellcraft/docs/soramech-notes.md and commit it,
> SoraMech's
> history contains the string of that path — anyone cloning SoraMech alone
> gets a dangling
...
> real copy.
> Because SoraMech's working tree is full of your uncommitted work right now,
> I'd build and
> maintain it through a dedicated worktree (a throwaway second checkout in a
> temp dir) so
...
> pool-ready so
  ...
  (phase 6)
> actually demand it. That's SoraMech's own v1-synchronous-→-pool path.
> Confirm, or do you
...
>    Three clean yeses and two design pivots — let me lock in what's ratified,
>      set up the docs branch the way you actually want it, and flag the one big
...
>           Hold on — I need to stop and investigate before touching branches.
>    Something's inconsistent: earlier my commit reported `[master 803c35c]` and
>   `git branch --show-current` said `master`, but now `branch -a` shows only `*
>       main`, and there's a worktree at `/mnt/mtwo/programs/sora/soramech`. The
>    `/home/ritz/...` and `/mnt/mtwo/...` paths, plus a master/main discrepancy,
>        mean I don't yet understand this repo's topology — and I won't create
>                                                                branches blind.

>   Now it's clear: `/home/ritz/programs/sora/soramech` is just a **symlink** to
>        `/mnt/mtwo/programs/sora/soramech` — same repo, one working tree. And
>           there's one local branch, **`main`** at my commit `803c35c`, intact.
>
>     One thing worth flagging, since git state is your domain: when I committed
>   earlier, git reported `[master 803c35c]` and `--show-current` said `master`.
>   Now the branch is `main` (no `master` exists). So between my commit and now,
>      `master` was renamed to `main` — likely you or your tooling, working in
>   parallel. Nothing lost; my doc commit is the tip of `main`. Just so you know
>                                                             it moved under me.
>
...
>
>     Creating the orphan `documentation` branch in an isolated worktree so your
>                                            dirty `main` tree is never touched:

>   Worktree's ready (unborn `documentation` branch, empty). Now let me populate
>       it, update pattern 5 to your resolved linked-list design, and record the
>                         ratified decisions into issue 101 — all in one pass:

>       Now recording the ratified decisions into issue 101 — Current Behavior
>                                                                         first:
...
>                     worktree; the `documentation` branch persists in the repo:

>          Done. The `documentation` orphan branch is committed (`773f713`), the
>  throwaway worktree is cleaned up, and your `main` tree is untouched — still
>        your 37 dirty changes. I notice `main` moved to `b3b2e86` ("the tracker
>         becomes whole…") while I worked; that's your double-gitters steaming
>  through, and my earlier clarification commit is safely back in its history. I
>                                              stayed entirely out of that lane.
>
>       One small accuracy fix to the notes' own "Sharing" section, since it's a
>                                                 symlink (pointer), not a copy:
...
## Your resorting design, repeated back
 ...
 - **The record store is an unsorted, append-only arena.** A new record lands at
       the next free slot — O(1), no shift, and existing records *never move*.
 - **Order lives in a separate link array.** Each element holds the index of the
    *next* element in sorted order — a singly-linked list threaded through the
               arena. Inserting is a link splice: append the record, repoint the
      predecessor's "next" at it, point its "next" at the old successor. No tail
                                                                    shift, ever.
- **Occasionally you resort by double-buffer:** walk the links in order, write a
   fresh compact contiguous sorted array off to the side, swap the consumer onto
            it. That reclaims freed holes and restores cache-friendly iteration.
...
independent, so insertion touches two links and one free slot, nothing else. And
 it *strengthens* the anti-tearing story: because records never move, a consumer
   ...
   1. **The insert is O(1); finding *where* to insert isn't.** You still have to
      ...
      objects move a little each frame, a cached "you were near here last frame"
     hint makes the walk short — and a fresh compaction gives you a contiguous
      array you can *binary-search* for the neighborhood. So compaction pays off
                            twice: cache-friendly reads *and* fast next-inserts.
...
**LuaJIT gives one Lua state per OS thread, and Lua values are not shared across
  states.** So the ring-buffer slot I just built and tested is a *single-thread*
  structure — it cannot be the shared wire between real threads. To honor "all
   separate threads with shared-memory slots," there are two faithful paths, and
                                                    this is genuinely your call:

        - **(B) A C shared-memory core — the *real* SoraMech architecture *(my
       recommendation)*.** Slots live in C shared memory guarded by atomics; box
      workers run on a C pool; the render thread reads the shared blackboard *in
              place* — matching your mental model exactly. The huge reuse win:
      **SoraMech's own `009-slot-store.c` already implements our three flavors**
                (FIFO ring, latest-wins peek, atomic fan-out counter) *plus* the
   ownership-handle-on-push contract, tested. We'd bind it via LuaJIT FFI rather
         than reinvent it. In this path, **the pure-Lua slot I wrote becomes the
  single-thread reference/test-double and the API contract** — not wasted, but
                                                          not the shipping wire.

  - **(A) lua-lanes message-passing threads.** Each box is its own LuaJIT state;
  slots become lindas (`send`/`receive` = FIFO, `set`/`get` = latest-wins). Pure
        Lua, no C to write, and copies-across-threads make tearing *structurally
            impossible*. But it's message-passing, not shared memory: the render
   blackboard becomes a channel you `get` a *copy* from each frame, not a shared
  array you sweep in place — a mismatch with how you pictured it, and raylib's
                                    FFI handles get confined to the render lane.
...
> contiguous array you can binary-search for the neighborhood. So compaction
> pays off twice:
...
>   Clear direction — and two good refinements. Let me take you up on studying
>     the actual SoraMech source before I commit to *how* we reuse it, since you
>     pointed me at it. Reading the slot store's real API and locating the pool:

>    C builds with `-fPIC` and `-pthread` — already shared-lib / FFI-friendly.
>               The pool lives at `libs/task-pool/pool.{c,h}`, the slot store at
...
>     **directly FFI-cdef-able**, built `-fPIC -pthread` already. That settles a
>   lot. Let me read the pool header to see how self-contained it is (whether it
>            can be vendored cleanly), before I recommend the exact reuse shape:
...
>         boxes later — but since you want C box functions, workers just run C
>                    function pointers directly. No Lua-in-threads complication.
>
...
**Lua for the architecture, C for the functions, FFI between them — that's the
right shape, and it's exactly what SoraMech itself does** (Lua defines/validates
             the graph; C is the pool + slots + dispatch). It also satisfies the
`note-to-claude-ai` methodology cleanly: the **Lua map is the readable "story"**
you can skim top-to-bottom (which boxes, which wires), and the **C box functions
   are the "very high index" arcane details** the reader skips unless they care.
                                                                     Concretely:

    - **Lua = the map.** Declares boxes, wires, allocates slots (via FFI), names
     each box's C function body, sets up the frame-clock, says "run." Iterate on
                                                     wiring without recompiling.
 - **C = the engine + the work.** The pool (separate threads), the shared-memory
            slots, the trigger-on-ready dispatch, and the box bodies (read-mice,
  solve-pose, cull, update-renderables) + the raylib render thread. Pool workers
          call C function pointers directly — no Lua living in worker threads.
- **FFI = the handoff.** Lua builds the map through FFI calls into C; C owns the
                                                      threads from "run" onward.

    **No, I wouldn't rather do it all in C.** Pure C would cost the readable-map
 story, the fast wiring iteration, and your stated global preference for Lua —
for no gain, since FFI already gives us C speed exactly where the hot path lives
  (slots, pool, box bodies, render). Lua-map + C-engine is strictly better here.
...
## The one thing to confirm before I pour C: the reuse shape
      ...
      You pointed me at the SoraMech source and I've now read the exact APIs. My
         ...
         self-contained (only pthreads), general, and tested. A thread pool is a
   thread pool; I'd drop a snapshot copy into this repo's `libs/task-pool/` with
     a provenance header, rather than depend on SoraMech's build (which is under
    active "double-gitter" flux). Its worker-init hook even leaves the door open
     for per-worker Lua states later, if phase-6 LLM boxes ever want Lua bodies.

  2. **The slots — write our own lean C, *modeled* on SoraMech's, not imported
     wholesale.** SoraMech's slot store is **byte-oriented** (`push(data, size)`
   with per-cell byte capacity) and carries machinery the game won't touch for a
      long time — dual-ring language-boundary values, unknown-size large-value
           heap + allocator, tagged pop. The game's wires are small **fixed-size
        structs** (a mouse delta, a renderable record). So I'd write a lean slot
    store that **copies SoraMech's hard-won, correct synchronization** (per-slot
     `atomic_flag` spinlock on the rings, `atomic_fetch_add` on the counter) but
         with typed fixed-size cells and only our three flavors — dropping the
    serialization/allocator/dual-ring baggage. The pure-Lua slot I already built
            stays as the single-thread **test double + API contract**; the C one
                                 implements the same contract with real locking.

          3. **The renderables blackboard (pattern 5's arena) — its own lean C
     structure**, separate from the slots (slots are box-to-box wires; the arena
                         is the presentation membrane the render thread sweeps).
...
byte/allocator/dual-ring complexity. This is the "lessons flow both ways through
    ...
    **Is that the reuse shape you want** — vendor the pool, lean-own the slots
  modeled on SoraMech's locking? Give me the nod (or tell me you'd rather import
 the full slot store, or depend on SoraMech directly) and I'll pour the C layer:
  vendor the pool, write the lean slot store with a matching FFI cdef, and stand
           up the first real box (`read-two-mice` → drain-and-sum) end to end.
...
> unknown-size
> large-value heap + allocator, tagged pop. The game's wires are small
> fixed-size structs (a mouse
...
> counter) but with typed fixed-size cells and only our three flavors —
> dropping the
> serialization/allocator/dual-ring baggage. The pure-Lua slot I already built
> stays as the
> single-thread test double + API contract; the C one implements the same
> contract with real
  locking.
...
> Is that the reuse shape you want — vendor the pool, lean-own the slots
> modeled on SoraMech's
> locking? Give me the nod (or tell me you'd rather import the full slot store,
> or depend on
...
>
>      **On effil / threading — you're right, just use C.** Effil (like lanes)
>    gives real OS threads but **not shared memory** — it copies values across
>  its own managed table proxies. Your engine wants *shared-memory* threads (the
>   in-place render blackboard, shared slots the pose box and render thread both
>    touch). No Lua threading library can do that; they all serialize across the
>     boundary. So **all threads live in C** (the pool + the render thread), the
> slots live in C shared memory with atomics, and **Lua never spawns a thread at
> all** — it's one state that builds the map and calls "run" through FFI, then
>     C owns every thread from there. That's why C is easier here: it's the only
>            layer that *can* do true shared memory. Effil isn't in the picture.
>
>      **On skipping SoraMech's slot features — let's do it one-by-one, as you
>                                                                        said:**
>
...
> | tagged pop (lowest-tag) | **skip** | only needed downstream of *parallel* iterators with nondeterministic push order — none yet |
> | large-value heap + allocator | **skip** | our cells are fixed-size structs; revisit at phase-6 LLM payloads |
> | dual-ring (native+JSON) | **skip** | that's cross-*language* marshaling; all our boxes are C, one language |
> | byte-blob cells | **replace** | with typed fixed-size struct cells — leaner, and a *locked* latest-slot peek can't even tear |
>
...
>                                                                          with.
>
>    **On the render consumer being a self-re-arming FIFO iterator** — got it,
>    and I've written it into pattern 7. One caveat I have to raise (I can build
> around it, just flagging): raylib's GL context is **thread-affine** — it can
>   only be driven from the one thread that created the window. So a render task
> that re-queues onto the pool and lands on a *different* worker each time can't
>       issue GL calls. The clean split: the **cull/renderables-rebuild** is the
>        self-re-arming, single-in-flight, FIFO pool iterator you described; the
>         **raylib draw** sits on its own dedicated GL-owning thread reading the
>    published buffer. Same "re-adds itself, one in flight" shape, just kept off
>  the pool because of GL affinity. Flagging so we decide it deliberately later;
>                                                      it doesn't block phase 1.
>
...
## Built and proven this turn (all uncommitted — we're mid-issue-101, commit on completion)
 ...
 - **`libs/engine-core/slot.{h,c}`** — the lean C slot store: typed fixed-size
      cells, three flavors, SoraMech-modeled per-slot spinlock + atomic counter.
                                  Compiles clean under `-Wall -Wextra -pthread`.
 - **`libs/engine-core/slot-test.c`** — regression prover, kept: single-thread
   contract + the two threaded tests that justify the whole drop-to-C (exactness
               under 8 producers, tear-free under concurrent writers). All pass.
                                                 - **`slot.info.md`** companion.
- **`docs/soramech-notes.md`** — now patterns 1–8, including your getter (8)
           and the self-re-arming FIFO render iterator + GL-affinity caveat (7).
   - Issue **101** records the ratified decision; the engine-foundation datapath
                                       carries a superseded banner; TOC updated.
...
## The path from here (I have your go-ahead — this is the next chunk)
  ...
  1. **The FFI bridge** — a tiny Makefile to build `slot.c` into a shared lib,
   an `ffi.cdef` of `slot.h`, and a thin Lua module wrapping it. Then re-run the
       contract test *through Lua* to prove the map layer can drive the C slots.
   This is the linchpin that validates "Lua map + C functions + FFI" end to end.
...
>  That's a real simplification, and it clarifies the whole shape. Let me record
>                 it and then be honest about the one thing it doesn't dissolve.
>
...
everything is uniformly a box and the only concession is that one attribute. (If
  you'd rather, the alternative is the main thread owns GL and drives the render
  box directly, but that reintroduces exactly the "special thread" you're trying
                                        to avoid — so I'd stick with the pin.)

## Impact on what's built

   - **The C slot store is unaffected** — it was already C, already tested. No
   rework. The FFI/Lua-binding step I was about to do is simply **deleted** from
                                      the plan. Nice side effect: less to build.
 - **The pure-Lua slot** (`src/soramech/000-ring-buffer-slot.lua` + its info) is
    now vestigial — it was the "Lua-side test-double," and there's no Lua side
  anymore. It's a legible spec of the contract, but so is `slot.info.md`. **Keep
   it as a reference, or should I remove it?** (It's uncommitted, so removing it
                                           loses nothing tracked — your call.)
...
> workers like the
> others. So instead of a bespoke render thread living outside the box model,
> you get one box with
...
> vestigial — it was
> the "Lua-side test-double," and there's no Lua side anymore. It's a legible
> spec of the
> contract, but so is slot.info.md. Keep it as a reference, or should I remove
> it? (It's
...
>    The C slot store (`libs/engine-core/slot.c`) is the finished translation of
>   that Lua reference — same three flavors, same contract, plus the threading
>   the Lua single-state version could never do. So the translation is complete;
>                                      removing the Lua originals as instructed:

>      Done — both instructions handled. The dedicated always-unblocked render
>   thread is actually the *simpler* path, and it needs zero new primitives: our
>      `slot_pop` already returns immediately when empty, so the render thread's
>  non-blocking "grab latest or redraw current" loop falls out of the slot store
>   as-is. And the Lua slot is gone now that `slot.c` carries its contract in C.
>                                        Records updated (pattern 7, issue 101).
>
>  Now I'll keep the momentum and pour the next piece — the worker pool. Going
>    **lean-own** (modeled on SoraMech's proven barrier/queue/quiescence design,
>  but tailored: no per-worker Lua-spec init, no priority machinery, built for a
...
>       Pool passes: **200k tasks each run once**, a task **re-armed itself 100k
>        times** (the heartbeat/iterator mechanism, proven), and **200k children
>    spawned from inside tasks** all ran. Clean build. The `.info.md` companion:

         Two tested substrate pieces now stand — the two hardest-to-get-right,
                                                                 threading-wise:

       - **`libs/engine-core/slot.{c,h}`** — the wires (shared-memory slots, 3
                                          flavors, tear-free under concurrency).
       - **`libs/task-pool/pool.{c,h}`** — the threads (workers running boxes;
                self-spawn proven, which *is* the heartbeat/iterator mechanism).
...
## What's left to make the graph actually turn
     ...
     1. **The dispatch / graph model** — the box + wire data structure and the
             **trigger-on-ready** rule: when a box's input slots are all filled,
    `pool_spawn` it; when it finishes, push its output down its wires (which may
        make *downstream* boxes ready). Plus iterator re-arm and the frame-clock
       heartbeat box. This is the piece that connects slots + pool into a living
          SoraMech map, and it's the heart of the architecture — worth its own
            focused pass, and testable with mock boxes before any real hardware.
 2. **The dedicated render thread** — the always-unblocked raylib loop reading
   ...
   3. **The boxes** — `read-two-mice → drain-and-sum → solve-pose → cull
      → render`. (`read-two-mice` is the one that needs real evdev devices, so
        it's the hardest to test headless — I'll build it against a mock input
                source first so the graph proves out without two physical mice.)
...
> one that needs real evdev devices, so it's the hardest to test headless —
> I'll build it against
...
> The dispatch/graph is the natural next chunk. Want me to keep going straight
> into it (building
> it against mock boxes so it's fully testable headless), or pause here on the
> two tested
  primitives?
...
>      Let's keep going — and I noted the commit discipline (only-my-files, at
>  issue boundaries; I'll follow it when issue 102 lands, which isn't yet). Your
...
>   authoritative independent sum. If they match, no delta was lost, duplicated,
>  or torn, no matter how the threads interleaved. That's the headless proof the
>                                                                   graph needs.
>
...
>             validates the whole chain at once — trigger-on-ready firing, the
>     single-spawn gate, the clear-then-recheck race fix, and drain-and-sum, all
>                            conserving every value under threaded interleaving.
>
>          But a concurrency test that passes *once* proves little — races are
>                                                intermittent. Let me hammer it:

>     40/40 clean — the single-spawn gate and clear-then-recheck race handling
>    hold up. The dataflow substrate is now proven end-to-end: **slots (wires) +
>    pool (threads) + dispatch (the graph turning)**, all three tested, the last
...
| Piece | What | Test |
|---|---|---|
| `libs/engine-core/slot.{c,h}` | the wires (shared-memory slots) | contract + threaded exactness + zero torn reads |
| `libs/task-pool/pool.{c,h}` | the threads (workers running boxes) | fan-out + self-spawn + nested spawn |
| `libs/engine-core/graph.{c,h}` | the graph turning (trigger-on-ready) | mouse round-trip, **40/40** under stress |
...
- **(A) Checkpoint now.** The substrate (slots + pool + dispatch) is a genuinely
         complete, tested unit. I carve it into a sub-issue **`102a — dataflow
           substrate`**, mark it done with the implementation record, move it to
        `completed/`, update the phase-1 progress index, and commit *only* those
                                   files. Clears the backlog at a real boundary.
      - **(B) Push to a runnable window first.** Keep building — the dedicated
    render thread + story `main()` + lifecycle bookends — until issue 102 as a
             whole is a runnable (if minimal) loop, then commit the whole engine
     foundation. Bigger commit, but a visibly-running one, closer to the phase-1
                                                                  demo you want.
...
> - (A) Checkpoint now. The substrate (slots + pool + dispatch) is a genuinely
> complete, tested
> unit. I carve it into a sub-issue 102a — dataflow substrate, mark it done
> with the
> implementation record, move it to completed/, update the phase-1 progress
> index, and commit only
...
>       On it. But first I have to check the git topology — the initial status
>      showed changes in sibling projects (`../../RPG-autobattler`, etc.), which
>    means this might be a shared parent repo, not a per-project one. Committing
>  "only my files" cleanly depends on knowing exactly where the repo root is and
>                                                            what else is dirty.
...
>     concurrent work. This is exactly why your "stage only my files by explicit
>  path" rule exists. I'll stage only my seven-or-so substrate files, verify the
>  staged set contains nothing else, and leave the other 570+ entries untouched.
>
...
>   Now staging **only** my substrate-concern files by explicit path — leaving
> the architecture-decision docs (issue 101, the datapath banner) for a separate
>               concern, and the other 570+ monorepo entries entirely untouched:

>        That staged list is a red flag — **398 files**, hundreds from *other*
>     projects (`world-edit-to-execute`, `delta-version`, `words-pdf`…). Those
>  were already staged by the concurrent "double-gitter" work *before* I touched
>      anything; my `git add` just piled my ~18 on top of their ~380. I must not
>  commit theirs, and — since you told me to let them steam through — I must
>     not *unstage* theirs either (that would trample their in-progress commit).
>
>    The right tool is a **pathspec commit**: `git commit -- <my paths>` records
...
## Checkpoint landed
  ...
  - **`a8c43d4a`** — the dataflow substrate (slots + pool + dispatch + tests +
    ...
    - **Left staged and untouched:** the ~381 concurrent-work entries across the
           monorepo — the double-gitters keep their in-progress commit intact.
        - **Left uncommitted on purpose** (my *architecture-decision* concern, a
        separate commit for later): the issue-101 edits and the datapath banner.
      Different concern from the substrate implementation, so I kept them out of
                                                                    this commit.
   - The untracked `llm-transcripts/` in the project is your export tooling, not
                                                            mine — left alone.
...
The **entire concurrent core of the engine is built, tested, and committed** —
   ...
   - **wires** (slots) · **threads** (pool) · **the graph turning** (dispatch)
       — all proven headless, the last by your self-checking mouse round-trip.
...
>                                                                 Good findings:
>  - **Transcripts:** just 3 small files (this session + the earlier Jul-7 one).
>                                                                Easy to commit.
>  - **raylib:** not in pkg-config, but `/usr/local/include/raylib.h` **exists**
>     — so it's installed locally. I'll verify it links before building on it.
...
>       against raylib, and run it with a frame budget so it auto-quits (opens a
>                                              window, draws, writes `goodbye`):

>         Clean build, but a `-Wformat-truncation` warning — and house rule is
>    "warnings are errors." The key buffer is pointless anyway (I only compare a
>                          prefix), so I'll compare the parsed pointer directly:
...
>             *executable* code belongs in the `/tmp` tier (which I confirmed is
>     exec-capable — the earlier runs launched from there). So the binary must
>       build into `tmp/` (the `/tmp` tier), not `tmp/shared-memory`. Fixing the
>                                                       Makefile and run script:

>     The whole build system works: `make test` runs all three substrate provers
>   green, and the `run` script builds + launches the game and writes `goodbye`.
>                                    The runnable loop is real and reproducible.
>
>        Now the sub-issue record (mirroring 102a), then I'll checkpoint-commit.
>                     Writing **102b** as the completed runnable-loop blueprint:
...
## Where phase 1 stands
    ...
    You can see it yourself: `./run` (no frame budget = runs until you close the
      ...
      - `issue 101` edits + the `datapath` banner — my *architecture-decision*
   concern, a separate commit I'll make when that's finalized (or on your word).
  - `output/goodbye` — a runtime artifact my runs rewrote; not mine to commit.
                        Worth gitignoring `output/` runtime files at some point.
...
>         mover bouncing inside real walls — which also seeds collision (105).
>  Starting with the world module's header, coordinate conventions written right
>                                                             on the structures:
...
>         All 16 world checks pass — coherent map, walls/floors/rooms/doors as
>       authored, dispatch firing (grid 20×12, 69 walls / 171 floor, 2 rooms, 1
>   door). Now making it **visible**: I'll wire the world into `main.c` — draw
>     it top-down, and turn the mover into a "player" that wanders the rooms and
>              bounces off real walls (the first taste of collision, issue 105).
...
>    Clean build (no warnings), ran and wrote goodbye. Now let me actually *see*
>  the render — adding a screenshot verb to the Platform seam so I can capture
>                                                           a frame and view it:

>  The screenshot didn't land where expected and the run exited non-zero — let
>      me diagnose directly (raylib's `TakeScreenshot` may resolve paths oddly),
>            capturing full output, the real exit code, and hunting for any PNG:
...
>        is the step-up), the perimeter and dividing walls in gray, the **door**
>      punched through the middle, and the player (yellow) wandering room 0. The
>   world model, its validator, the top-down debug view, and the first collision
>                                                                      all work.
>
>    The `world-view.png` is a regenerable debug artifact (via `FPS_SHOT`), so I
>   won't commit it — I'll clean it up. Now marking 103 complete and recording
>                                          the lessons. First the issue updates:

>          The world module needs its `.info.md` (house rule), then the lessons:

>    Now the lessons learned — to the strategems (proven dataflow shapes), and
>                                   the desire/faith directories in their voice:

>    Updating the phase-1 progress index for 103, then moving it to `completed/`
>                                          and cleaning up the stray screenshot:
...
>     grew them, so "every commit" holds). The only things left unstaged are the
...
## This turn's work: issue 103 complete
 ...
 - **`src/001-world.{c,h}`** — the map as two lenses: a tile grid (solid, wall
   id, floor/ceiling height, room id) for renderer + collision, and a room table
            + door graph for gameplay. Plus the **room-behaviour dispatch seam**
     (enter/step/exit — the Phase 4/6 hook), a hand-authored two-room test map
                with a door and a step-up, and a validator. 16 world-tests pass.
  - **Made visible** — the render thread draws it top-down, the player wanders
   both rooms and **bounces off real walls** (the first taste of 105 collision).
    You saw the screenshot: two rooms, the dividing wall, the door, the step-up,
                                                                     the player.
                       - **Lessons written** to the three directories, as asked:
                  `strategems/dataflow-shapes.md` (the self-checking round-trip,
      publish-with-one-write, trigger-on-ready, two handoff disciplines), plus a
     `desire/` wish (a true timer box instead of the borrowed `nanosleep`) and a
          `faith/` expectation (the one dataflow shape carrying the whole game).
...
so, I was looking at the engine renderer for the world, and I think we need to
rethink the design a lot. It doesn't match the vision. So, here's my suggestion
- we make a 3d world that stores vertices, and each one is stored on a
datastructure per object. We can have multiple datastructures for objects with
large amounts of vertices. Then, we track normals (as in, which way the surface
is pointing to the player) and we apply colors to the edges. We only render, to
the player, the vertices that are visible to them, and we can track this with
just a raycast from the object to various cameras. Then, we can apply sharp and
diffuse lighting effects if we so choose. but that's for another day. for now,
we need to make it so that the player is a camera floating in a room with many
doors and rooms. sorta like the world-demo you just made, except instead of a
top-down perspective it's first person through the camera as the camera follows
the gameobject of the player through the realm.

--------------------------------------------------------------------------------

### Assistant Response 17

>           You're right — the top-down view was a debug crutch; the vision is
>   first-person, and we should build the real thing. Let me engage your design,
>             flag one technical fork, then start putting a camera in the rooms.
>
> ## Your 3D design — and one thing worth deciding
>
> I follow it: **per-object vertex datastructures** (chunked for heavy objects),
>     **normals** (surface-vs-viewer direction), **colored edges**, **visibility
>     culling**, lighting later. The per-object vertex structure and normals and
>            edge-coloring are all good and I'll build with them from the start.
>
>    The one fork I want to name, because it saves real work: **"only render the
>     vertices visible to the player, via a raycast."** Modern GPUs (what raylib
> drives) already solve *per-pixel* occlusion for free with the **depth buffer**
>   — you draw all your geometry and the GPU keeps the nearest surface at each
>      pixel; nothing behind a wall shows through. So hand-raycasting per-vertex
>             visibility is mostly redundant with hardware you're already using.
>
>      Where the *idea* genuinely pays off is at the **coarse** level, and for a
>     world of **rooms joined by doors** it has a beautiful exact form: **portal
>   culling.** Each door is a portal; you draw the room you're in, then for each
>  visible door you draw the next room *clipped to that door's shape on screen*,
>     recursively. That's "only render what's visible through the openings" done
> precisely — and it's the elegant version of your raycast instinct, fitted to
>                                                                    this world.
>
> **My recommendation:** let the GPU depth buffer handle occlusion now (draw the
>       rooms, frustum-cull what's off-screen), and add **portal culling through
> doors** when the realm gets big enough that drawing everything is wasteful. If
>   you specifically want the raycast/portal *character* from the start (it does
>   fit the vision's "measuring angles" theme), say so and I'll build the portal
>        walk instead. Either way the geometry (per-room vertex meshes, normals,
>                          colored edges) is the same, so I can start on it now.
>
>  Starting with the base first-person view. Extending the Platform seam with 3D
>                                   verbs (keeping raylib out of the interface):

> Implementing the 3D verbs in raylib (depth-buffer occlusion; drawing all faces
>                              for now, before lighting/backface-culling lands):

>  Now the renderer module — turning the world's tiles into per-room 3D meshes
>      (vertices, normals, colored edges) and drawing them first-person. Header:

>  Now wiring the renderer into `main.c` — the renderable becomes the player's
>       eye state (position, height, facing), the mover computes facing from its
> travel direction, and the render thread flies a first-person camera instead of
>                                                              drawing top-down:

> Now the render thread — replacing the top-down drawing with the first-person
>                                                                  scene render:

>    The `main()` body — build the scene from the world, wire it to the render
>                                                      thread, and tear it down:

>                                              Adding the renderer to the build:

> Clean build, clean run, screenshot captured. Let me see the first-person view:

>       **That's first-person.** A real 3D room from the eye: green tiled floor,
>       gray-blue walls, dark ceiling top-right, all outlined in the bright cyan
> **colored edges** you asked for — and the perspective/depth is the GPU depth
>     buffer resolving what's in front of what. The camera is flying through the
>   world following the auto-wandering player, built from per-room vertex meshes
>        (with normals stored for the lighting to come). Your design, on screen.
>
>      It's showing one room's corner, though — you asked for **many rooms and
> doors**. Let me expand the world to a proper little realm so there's something
> to fly through. Rewriting the test world into four rooms joined by four doors,
>                                                     with varied floor heights:

>                                 Updating the world test for the 4-room layout:

>                 Rebuild, verify the 4-room model, and capture the fly-through:

>  The 4-room realm validates (4 rooms, 4 doors, 349 open tiles). Let me see the
>                                                              fly-through view:

>      That caught a bad angle — the wandering player reached a corner and the
>   camera's looking almost straight into a wall (the flat fill with a sliver of
>     floor). The render's fine (the first shot proved it); it's just screenshot
> timing on the auto-wander. Let me make the capture frame configurable and grab
>                                                     an earlier, centered view:

>             That's a clean first-person view — green tiled floor receding in
>       perspective, two walls meeting at a corner, all edged in cyan. The realm
> renders first-person exactly as designed. (It's showing a corner rather than a
>    doorway, but the 3D world, depth, and colored-edge look are all confirmed.)
>
>  Let me record the renderer design decision. Reading the old 104a to supersede
>                                                      it with your 3D approach:

>      Superseding 104a with your 3D vertex approach — Current Behavior first:

                                                 Adding the status line to 104a:
