...
Generated on: 2026-09-08 10:59:28
...
**On effil / threading — you're right, just use C.** Effil (like lanes) gives
real OS threads but **not shared memory** — it copies values across its own
managed table proxies. Your engine wants *shared-memory* threads (the in-place
render blackboard, shared slots the pose box and render thread both touch). No
Lua threading library can do that; they all serialize across the boundary. So
**all threads live in C** (the pool + the render thread), the slots live in C
shared memory with atomics, and **Lua never spawns a thread at all** — it's
one state that builds the map and calls "run" through FFI, then C owns every
thread from there. That's why C is easier here: it's the only layer that *can*
do true shared memory. Effil isn't in the picture.
...
**On the render consumer being a self-re-arming FIFO iterator** — got it, and
I've written it into pattern 7. One caveat I have to raise (I can build around
it, just flagging): raylib's GL context is **thread-affine** — it can only be
driven from the one thread that created the window. So a render task that
re-queues onto the pool and lands on a *different* worker each time can't issue
GL calls. The clean split: the **cull/renderables-rebuild** is the
self-re-arming, single-in-flight, FIFO pool iterator you described; the **raylib
draw** sits on its own dedicated GL-owning thread reading the published buffer.
Same "re-adds itself, one in flight" shape, just kept off the pool because of GL
affinity. Flagging so we decide it deliberately later; it doesn't block phase 1.
...
already went out without it, let me catch it up now. But first let me see what's
actually in there (and check raylib, since it's about to be our first real
dependency):
...
Where the *idea* genuinely pays off is at the **coarse** level, and for a world
of **rooms joined by doors** it has a beautiful exact form: **portal culling.**
Each door is a portal; you draw the room you're in, then for each visible door
you draw the next room *clipped to that door's shape on screen*, recursively.
That's "only render what's visible through the openings" done precisely — and
it's the elegant version of your raycast instinct, fitted to this world.

**My recommendation:** let the GPU depth buffer handle occlusion now (draw the
rooms, frustum-cull what's off-screen), and add **portal culling through doors**
when the realm gets big enough that drawing everything is wasteful. If you
specifically want the raycast/portal *character* from the start (it does fit the
vision's "measuring angles" theme), say so and I'll build the portal walk
instead. Either way the geometry (per-room vertex meshes, normals, colored
edges) is the same, so I can start on it now.
...
eye state (position, height, facing), the mover computes facing from its travel
direction, and the render thread flies a first-person camera instead of drawing
top-down:
...
buffer resolving what's in front of what. The camera is flying through the world
following the auto-wandering player, built from per-room vertex meshes (with
normals stored for the lighting to come). Your design, on screen.
...
That's a clean first-person view — green tiled floor receding in perspective,
two walls meeting at a corner, all edged in cyan. The realm renders first-person
exactly as designed. (It's showing a corner rather than a doorway, but the 3D
world, depth, and colored-edge look are all confirmed.)

Let me record the renderer design decision. Reading the old 104a to supersede it
with your 3D approach:
