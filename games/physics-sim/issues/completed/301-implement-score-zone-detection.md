...
Implemented ball_check_zone() function following the suggested design:

1. Added function declaration to src/006-ball.h:235
   - Function signature matches specification
   - Returns zone index (0 to zone_count-1) or -1

2. Implemented ball_check_zone() in src/007-ball.c:412
   - Validates ball and world pointers
   - Checks if ball is active
   - Checks if ball->y > ZONE_TOP_Y (560.0f)
   - Loops through zones checking x boundaries
   - Returns first matching zone index

3. Updated src/006-ball.info.md:170
   - Added full API documentation
   - Documented thread-safety (read-only function)
   - Explained detection logic

Detection implementation uses ball center point for zone checking. Thread-safe design allows calling from parallel ball update tasks. Function will be integrated with scoring system in Issue 502.
